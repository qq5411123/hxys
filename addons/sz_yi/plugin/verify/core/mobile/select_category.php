...
if ($operation == 'category') {
	if (!empty($_GPC['level'])) {
		...
		if (!empty($goodsids)) {
			$parent_category = pdo_fetchall('SELECT a.id,a.parentid,a.name,a.level FROM ' . tablename('sz_yi_category') . ' a LEFT JOIN  ' . tablename('sz_yi_goods') . ' b ON (a.id = b.pcate)  WHERE a.parentid=0 AND a.uniacid=:uniacid and b.id IN (' . implode(',', $goodsids) . ') GROUP BY a.id', array(':uniacid' => $_W['uniacid']));
		}

...
class Address extends \admin\api\YZ
{
	...
	public function save()
	{
		...
		if (!empty($id)) {
			...
			$item = pdo_fetch('SELECT address FROM ' . tablename('sz_yi_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $para['uniacid']));
			...
			pdo_update('sz_yi_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $para['uniacid']));

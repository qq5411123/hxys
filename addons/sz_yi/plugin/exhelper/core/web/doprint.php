...
if ($op == 'search') {
	...
	if ($_W['ispost']) {
		...
		if (empty($arr) || empty($pt)) {
			exit(json_encode(array('result' => 'error', 'resp' => '数据错误')));
		}
...
else if ($op == 'getPrintTemp') {
	if ($_W['ispost']) {
		$type = intval($_GPC['type']);

		if (empty($type)) {
			exit(json_encode(array('result' => 'error', 'resp' => '加载模版错误! 请刷新重试。')));
		}
		...
		if (!is_array($expDatas)) {
			exit(json_encode(array('result' => 'error', 'resp' => '请先设置默认打印模版！')));
		}
		...
		exit(json_encode(array('result' => 'success', 'respDatas' => $expDatas, 'respUser' => $expSendTemp, 'respTemp' => $expTemp)));
	}
}
else if ($op == 'getOrderState') {
	if ($_W['ispost']) {
		...
		exit(json_encode(array('printTemp' => $printTemp, 'datas' => $arr)));
	}
}
else if ($op == 'dosend') {
	if ($_W['ispost']) {
	...
	if ($op == 'saveaddress') {
		if ($_W['ispost']) {
			...
			if (empty($ordersns)) {
				exit(json_encode(array('result' => 'error', 'resp' => '订单数据为空')));
			}

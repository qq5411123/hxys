...
if ($operation == 'display') {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		foreach ($_GPC['displayorder'] as $id => $displayorder) {
			pdo_update('sz_yi_adv', array('displayorder' => $displayorder), array('id' => $id));
		}
	...
	if (checksubmit('submit')) {
		$data = array('uniacid' => $_W['uniacid'], 'advname' => trim($_GPC['advname']), 'link' => trim($_GPC['link']), 'enabled' => intval($_GPC['enabled']), 'displayorder' => intval($_GPC['displayorder']), 'thumb' => save_media($_GPC['thumb']), 'thumb_pc' => save_media($_GPC['thumb_pc']));

		if (!empty($id)) {
			pdo_update('sz_yi_adv', $data, array('id' => $id));
	...
	$item = pdo_fetch('select * from ' . tablename('sz_yi_adv') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
}
else {
	if ($operation == 'delete') {
		ca('shop.adv.delete');
		...
		pdo_delete('sz_yi_adv', array('id' => $id));

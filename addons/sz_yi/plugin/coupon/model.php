...
if (!class_exists('CouponModel')) {
	class CouponModel extends PluginModel
	{
		...
		public function creditshop($logid = 0)
		{
			global $_W;
			global $_GPC;
			$_obf_DSgPLRoTBDQiFhIfPggaGSwwPRc9NiI_ = p('creditshop');

			if (!$_obf_DSgPLRoTBDQiFhIfPggaGSwwPRc9NiI_) {
				return NULL;
			}
			...
			if (!empty($log)) {
				...
				$goods = $_obf_DSgPLRoTBDQiFhIfPggaGSwwPRc9NiI_->getGoods($log['couponid'], $member);
				$_obf_DUA7DFwZGgI4LxcxGCc4FjBcOzsZGjI_ = array('uniacid' => $_W['uniacid'], 'openid' => $log['openid'], 'logno' => m('common')->createNO('coupon_log', 'logno', 'CC'), 'couponid' => $log['couponid'], 'status' => 1, 'paystatus' => 0 < $goods['money'] ? 0 : -1, 'creditstatus' => 0 < $goods['credit'] ? 0 : -1, 'createtime' => time(), 'getfrom' => 2);
				pdo_insert('sz_yi_coupon_log', $_obf_DUA7DFwZGgI4LxcxGCc4FjBcOzsZGjI_);
		...
		public function poster($member, $couponid, $couponnum)
		{
			global $_W;
			global $_GPC;
			$_obf_DR4SMxoYAyMKHxA4BTwTXDQMQC03DCI_ = p('poster');

			if (!$_obf_DR4SMxoYAyMKHxA4BTwTXDQMQC03DCI_) {
				return NULL;
			}
			...
			while ($i <= $couponnum) {
				$_obf_DUA7DFwZGgI4LxcxGCc4FjBcOzsZGjI_ = array('uniacid' => $_W['uniacid'], 'openid' => $member['openid'], 'logno' => m('common')->createNO('coupon_log', 'logno', 'CC'), 'couponid' => $couponid, 'status' => 1, 'paystatus' => -1, 'creditstatus' => -1, 'createtime' => time(), 'getfrom' => 3);
				pdo_insert('sz_yi_coupon_log', $_obf_DUA7DFwZGgI4LxcxGCc4FjBcOzsZGjI_);
		...
		public function sendBackMessage($openid, $coupon, $gives)
		{
			...
			$_obf_DQk_Ays7OzMwQBs7OSMCMicNKA8DLzI_ = '';

			if (isset($gives['credit'])) {
				$_obf_DQk_Ays7OzMwQBs7OSMCMicNKA8DLzI_ .= ' ' . $gives['credit'] . '个积分';
			}

			if (isset($gives['money'])) {
				if (!empty($_obf_DQk_Ays7OzMwQBs7OSMCMicNKA8DLzI_)) {
					$_obf_DQk_Ays7OzMwQBs7OSMCMicNKA8DLzI_ .= '，';
				}

				$_obf_DQk_Ays7OzMwQBs7OSMCMicNKA8DLzI_ .= $gives['money'] . '元余额';
			}
			...
			if (isset($gives['redpack'])) {
				...
				$_obf_DQk_Ays7OzMwQBs7OSMCMicNKA8DLzI_ .= $gives['redpack'] . '元现金';
			}

			$content .= $_obf_DQk_Ays7OzMwQBs7OSMCMicNKA8DLzI_;
			$content .= '，请查看您的账户，谢谢!';
			$msg = array(
		...
		public function consumeCouponCount($openid, $enough = 0, $supplier_uid = 0, $sid = 0, $iscashier = 0, $goodid = 0, $cartid = 0, $coupon_carrierid = 0)
		{
			global $_W;
			global $_GPC;
			$time = time();
			...
			if ($iscashier == 1) {
				...
				$_obf_DQc3FyodJlwGMSEXGyo0HwQBEzA8KDI_ = pdo_fetchall($sqlcount, array(':openid' => $openid, ':uniacid' => $_W['uniacid']));

				foreach ($_obf_DQc3FyodJlwGMSEXGyo0HwQBEzA8KDI_ as $value) {
			...
			$total = pdo_fetchcolumn($sqlcount, array(':openid' => $openid, ':uniacid' => $_W['uniacid']));
			$sql = 'select * from ' . tablename('sz_yi_coupon_data') . ' d ' . '  left join ' . tablename('sz_yi_coupon') . ' c on d.couponid = c.id ' . '  where d.openid=:openid and d.uniacid=:uniacid and c.getcashier=0 and c.coupontype=0 and ' . $enough . '>=c.enough and d.used=0 ' . ' and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timestart<=' . $time . ' && c.timeend>=' . $time . '))';
			$_obf_DQc3FyodJlwGMSEXGyo0HwQBEzA8KDI_ = pdo_fetchall($sql, array(':openid' => $openid, ':uniacid' => $_W['uniacid']));

			foreach ($_obf_DQc3FyodJlwGMSEXGyo0HwQBEzA8KDI_ as $row) {
						...
						if (!empty($supplierids) && ($row['getsupplier'] == 1)) {
							...
							if ($row['usetype'] == 2) {
								foreach ($cartids as $key => $value) {
									...
									$_obf_DSQULywWDC0_CRQJKjQyAw8ZKTMmOxE_ = pdo_fetchcolumn('SELECT supplier_uid FROM ' . tablename('sz_yi_goods') . ' WHERE id=' . $gid);

									if (!empty($_obf_DSQULywWDC0_CRQJKjQyAw8ZKTMmOxE_)) {
										unset($cartids[$key]);
									}
								}
								...
								if ($row['usetype'] == 1) {
									if (!empty($categoryids)) {
										foreach ($categoryids as $v) {
											foreach ($_obf_DTIROCIZAhg_DhQhLRJcJiweDQQpMBE_ as $vc) {
		...
		public function useConsumeCoupon($logid5 = 0)
		{
			global $_W;
			global $_GPC;
		...
		public function backConsumeCoupon($order)
		{
			...
			$gives = array();
			$_obf_DTEvEDxbPTYFFgMODw0aMRAmKScjCAE_ = false;
			if (($order['status'] == 1) && ($coupon['backwhen'] == 2)) {
				$_obf_DTEvEDxbPTYFFgMODw0aMRAmKScjCAE_ = true;
			}
			else {
				if ($order['status'] == 3) {
			...
			if ($_obf_DTEvEDxbPTYFFgMODw0aMRAmKScjCAE_) {
	...
	'coupon' => array(
		...
		'isplugin' => true,
		'child'    => array(
			'coupon'   => array('text' => '优惠券', 'add' => '添加优惠券-log', 'edit' => '编辑优惠券-log', 'delete' => '删除优惠券-log', 'send' => '发放优惠券-log'),
			'category' => array('text' => '分类', 'add' => '添加分类-log', 'edit' => '编辑分类-log', 'delete' => '删除分类-log'),

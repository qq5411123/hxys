...
class PosterProcessor extends PluginProcessor
{
	...
	private function responseScan($obj)
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('sz_yi_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
	...
	private function responseSubscribe($obj)
	{
		...
		if ($member['isnew']) {
			pdo_update('sz_yi_poster', array('follows' => $poster['follows'] + 1), array('id' => $poster['id']));
		}

		$qrmember = m('member')->getMember($qr['openid']);
		$log = pdo_fetch('select * from ' . tablename('sz_yi_poster_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
		if (empty($log) && ($openid != $qr['openid'])) {
			...
			$_obf_DSoVMykoIwsSMQQKNz0PBxIqLRo3PxE_ = false;
			$issub = false;
			$pcoupon = p('coupon');
			...
			if ($pcoupon) {
				if (!empty($poster['reccouponid']) && (0 < $poster['reccouponnum'])) {
					...
					if (!empty($reccoupon)) {
						$_obf_DSoVMykoIwsSMQQKNz0PBxIqLRo3PxE_ = true;
					}
				}
	...
	'remark'   => array('value' => "\r\n谢谢您对我们的支持！", 'color' => '#4a5077')
			...
			if ($_obf_DSoVMykoIwsSMQQKNz0PBxIqLRo3PxE_) {

...
if (($store['deredpack'] == 1) && ($store['decommission'] == 1) && ($store['decredits'] == 1)) {
	...
	if (($store['deredpack'] == 1) && ($store['decommission'] == 1)) {
		$text = '奖励红包和佣金费用';
	}
	else {
		if (($store['decommission'] == 1) && ($store['decredits'] == 1)) {
			$text = '佣金和奖励余额费用';
		}
		else {
			if (($store['deredpack'] == 1) && ($store['decredits'] == 1)) {
				$text = '奖励红包和奖励余额费用';
			}
...
if ($_W['isajax']) {
	if ($_W['ispost'] && ($_GPC['op'] == 'sub_info')) {
		$data = array('uniacid' => $_W['uniacid'], 'name' => trim($_GPC['name']), 'contact' => trim($_GPC['contact']), 'mobile' => trim($_GPC['mobile']), 'address' => trim($_GPC['address']), 'deduct_credit1' => trim($_GPC['deduct_credit1']), 'deduct_credit2' => trim($_GPC['deduct_credit2']), 'commission1_rate' => trim($_GPC['commission1_rate']), 'commission2_rate' => trim($_GPC['commission2_rate']), 'commission3_rate' => trim($_GPC['commission3_rate']), 'credit1' => trim($_GPC['credit1']), 'redpack_min' => trim($_GPC['redpack_min']), 'redpack' => trim($_GPC['redpack']), 'creditpack' => trim($_GPC['creditpack']));

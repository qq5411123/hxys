...
{
	$setdata = pdo_fetch('select * from ' . tablename('sz_yi_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $wechatid));
	$plugins = iunserializer($setdata['plugins']);
	if (is_array($plugins) && isset($plugins[$identity])) {
		$newset = pdo_fetch('select * from ' . tablename('sz_yi_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $wechatid1));

		if (empty($newset)) {
			$newplugins = array($identity => $plugins[$identity]);
			pdo_insert('sz_yi_sysset', array('plugins' => iserializer($newplugins), 'sec' => iserializer(array()), 'sets' => iserializer(array()), 'uniacid' => $wechatid1));
		}
		else {
			$newplugins = iunserializer($newset['plugins']);
			$newplugins[$identity] = $plugins[$identity];
			pdo_update('sz_yi_sysset', array('plugins' => iserializer($newplugins)), array('uniacid' => $wechatid1));
		}

		$set = pdo_fetch('select * from ' . tablename('sz_yi_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $wechatid1));
		m('cache')->set('sysset', $set, $wechatid1);

		if ($transtype == 1) {
			unset($plugins[$identity]);
			pdo_update('sz_yi_sysset', array('plugins' => iserializer($plugins[$identity])), array('uniacid' => $wechatid));
		}
	}
}
...
if (checksubmit('submit')) {
	...
	if (is_array($_GPC['shop'])) {
		foreach ($_GPC['shop'] as $data) {
			if ($data == 'goods') {
				...
				$goods = pdo_fetchall('select * from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid', array(':uniacid' => $wechatid));
				...
				foreach ($goods as $g) {
					...
					$params = pdo_fetchall('select * from ' . tablename('sz_yi_goods_param') . ' where goodsid=:goodsid and uniacid=:uniacid', array(':uniacid' => $wechatid, ':goodsid' => $goodsid));
					...
					$newspecs = array();
					$specs = pdo_fetchall('select * from ' . tablename('sz_yi_goods_spec') . ' where goodsid=:goodsid and uniacid=:uniacid', array(':uniacid' => $wechatid, ':goodsid' => $goodsid));
					...
					$options = pdo_fetchall('select * from ' . tablename('sz_yi_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid ', array(':uniacid' => $wechatid, ':goodsid' => $goodsid));
					...
					foreach ($options as $o) {
						...
						pdo_update('sz_yi_goods_option', array('specs' => implode('_', $newspecitemids)), array('id' => $newoptionid));
					}
				}
				...
				$ccates1 = array();
				$tcates1 = array();
				$pcates = pdo_fetchall('select * from ' . tablename('sz_yi_category') . ' where uniacid=:uniacid and parentid=0', array(':uniacid' => $wechatid));
				...
				foreach ($pcates as $pcate) {
					...
					pdo_update('sz_yi_goods', array('pcate' => $newpcateid), array('uniacid' => $wechatid1, 'pcate' => $pcateid));
					$ccates = pdo_fetchall('select * from ' . tablename('sz_yi_category') . ' where parentid=:parentid ', array(':parentid' => $pcateid));
					...
					foreach ($ccates as $ccate) {
						...
						pdo_update('sz_yi_goods', array('ccate' => $newpcateid), array('uniacid' => $wechatid1, 'ccate' => $ccateid));
						$newccateid = pdo_insertid();
						$tcates = pdo_fetchall('select * from ' . tablename('sz_yi_category') . ' where parentid=:parentid ', array(':parentid' => $ccateid));
						...
						foreach ($tcates as $tcate) {
							...
							pdo_update('sz_yi_goods', array('tcate' => $newtcateid), array('uniacid' => $wechatid1, 'tcate' => $tcateid));
						}
					}
				}

				$goods = pdo_fetchall('select id,pcates,ccates,tcates from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid', array(':uniacid' => $wechatid1));
				...
				foreach ($goods as $g) {
					...
					pdo_update('sz_yi_goods', array('pcates' => implode(',', $newpcates), 'ccates' => implode(',', $newccates), 'tcates' => implode(',', $newtcates)), array('id' => $g['id']));
				}
	...
	if (is_array($_GPC['creditshop'])) {
		foreach ($_GPC['creditshop'] as $data) {
			if ($data == 'goods') {
				...
				$cates = pdo_fetchall('select * from ' . tablename('sz_yi_creditshop_category') . ' where uniacid=:uniacid', array(':uniacid' => $wechatid));
				$categoodsids = array();
				...
				foreach ($cates as $cate) {
					...
					$goods = pdo_fetchall('select * from ' . tablename('sz_yi_creditshop_goods') . ' where uniacid=:uniacid and cate=:cate', array(':uniacid' => $wechatid, ':cate' => $cateid));
				...
				if (!empty($categoodsids)) {
					$goods = pdo_fetchall('select * from ' . tablename('sz_yi_creditshop_goods') . ' where uniacid=:uniacid and id not in (' . implode(',', $categoodsids) . ')', array(':uniacid' => $wechatid));
	...
	if (is_array($_GPC['virtual'])) {
		foreach ($_GPC['virtual'] as $data) {
			...
			$cates = pdo_fetchall('select * from ' . tablename('sz_yi_virtual_category') . ' where uniacid=:uniacid', array(':uniacid' => $wechatid));
			$catevirtuals = array();
			...
			foreach ($cates as $cate) {
				...
				$newcateid = pdo_insertid();
				$virtuals = pdo_fetchall('select * from ' . tablename('sz_yi_virtual_type') . ' where uniacid=:uniacid and cate=:cate', array(':uniacid' => $wechatid, ':cate' => $cateid));

				foreach ($virtuals as $g) {
					$goodsid = $g['id'];
					$datas = pdo_fetchall('select * from ' . tablename('sz_yi_virtual_data') . ' where uniacid=:uniacid and typeid=:typeid and usetime=0', array(':uniacid' => $wechatid, ':typeid' => $goodsid));
			...
			if (!empty($catevirtuals)) {
				$virtuals = pdo_fetchall('select * from ' . tablename('sz_yi_virtual_type') . ' where uniacid=:uniacid and id not in (' . implode(',', $catevirtuals) . ')', array(':uniacid' => $wechatid));
	...
	if (is_array($_GPC['article'])) {
		foreach ($_GPC['article'] as $data) {
			if ($data == 'article') {
				...
				$cates = pdo_fetchall('select * from ' . tablename('sz_yi_article_category') . ' where uniacid=:uniacid', array(':uniacid' => $wechatid));
				$catearticles = array();
				...
				foreach ($cates as $cate) {
					...
					$newcateid = pdo_insertid();
					$articles = pdo_fetchall('select * from ' . tablename('sz_yi_article') . ' where uniacid=:uniacid and article_category=:article_category', array(':uniacid' => $wechatid, ':article_category' => $cateid));
				...
				if (!empty($catearticles)) {
					$articles = pdo_fetchall('select * from ' . tablename('sz_yi_article') . ' where uniacid=:uniacid and id not in (' . implode(',', $catearticles) . ')', array(':uniacid' => $wechatid));
	...
	if (is_array($_GPC['coupon'])) {
		foreach ($_GPC['coupon'] as $data) {
			if ($data == 'coupon') {
				...
				$cates = pdo_fetchall('select * from ' . tablename('sz_yi_coupon_category') . ' where uniacid=:uniacid', array(':uniacid' => $wechatid));
				$catecoupons = array();
				...
				foreach ($cates as $cate) {
					...
					$newcateid = pdo_insertid();
					$coupons = pdo_fetchall('select * from ' . tablename('sz_yi_coupon') . ' where uniacid=:uniacid and catid=:catid', array(':uniacid' => $wechatid, ':catid' => $cateid));
				...
				if (!empty($catecoupons)) {
					$coupons = pdo_fetchall('select * from ' . tablename('sz_yi_coupon') . ' where uniacid=:uniacid and id not in (' . implode(',', $catecoupons) . ')', array(':uniacid' => $wechatid));

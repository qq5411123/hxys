...
if (($operation == 'display') && $_W['isajax']) {
	...
	if (empty($log)) {
		$log = array('uniacid' => $uniacid, 'openid' => $member['openid'], 'module' => 'sz_yi', 'tid' => $order['ordersn_general'], 'fee' => $order['price'], 'status' => 0);
	...
	if (($operation == 'pay') && $_W['ispost']) {
		...
		if (($operation == 'complete') && $_W['ispost']) {
			...
			$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'sz_yi', ':tid' => $order['pay_ordersn']));
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('sz_yi_order') . ' where ordersn_general=:ordersn and uniacid=:uniacid', array(':ordersn' => $log['tid'], ':uniacid' => $_W['uniacid']));
			$store = pdo_fetch('select * from ' . tablename('sz_yi_cashier_order') . ' o inner join ' . tablename('sz_yi_cashier_store') . ' s on o.cashier_store_id = s.id where o.order_id=:orderid and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
			if (p('coupon') && (0 < $store['coupon_id'])) {

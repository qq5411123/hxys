...
if ($_W['isajax'] && $_W['ispost']) {
	...
	if ($apido == 'selectcashier') {
		...
		exit(json_encode($cashier));
		return 1;
	}

	if ($apido == 'postcategory') {
		$cid = intval($_GPC['cid']);
		$cname = $_GPC['cname'];
		...
		if (!empty($cname)) {
			...
			if (!empty($cates)) {
				exit(json_encode(array('result' => 'error', 'desc' => '分类名称已存在')));
			}

			$arr = array('category_name' => $cname, 'uniacid' => $_W['uniacid'], 'm_level' => $m_level, 'd_level' => $d_level, 'loveshow' => $loveshow);
			...
			if (empty($cid)) {
				...
				exit(json_encode(array('result' => 'success-add', 'cid' => $insertid, 'cname' => $cname, 'm_level' => $m_level, 'd_level' => $d_level, 'loveshow' => $loveshow)));
				return 1;
			}

			ca('article.cate.editcate');
			pdo_update('sz_yi_article_category', $arr, array('id' => $cid));
			exit(json_encode(array('result' => 'success-edit', 'cid' => $cid, 'cname' => $cname, 'm_level' => $m_level, 'd_level' => $d_level, 'loveshow' => $loveshow)));
			return 1;
		}

		exit(json_encode(array('result' => 'error', 'desc' => '分类名称为空')));
		return 1;
	}
		...
		if (is_array($data)) {
			...
			if (!empty($arr['id'])) {
				...
				if ($arr['article_keyword'] != $articlekeyword) {
					...
					if (!empty($keyword)) {
						exit(json_encode(array('result' => 'error-key', 'desc' => '关键字已存在!')));
					}
				}
			}
			else {
			...
			if (empty($arr['id'])) {
				...
				if (p('love')) {
					if (0 < $love_money) {
						$data = array('money' => $love_money, 'createtime' => time(), 'uniacid' => $_W['uniacid'], 'status' => 1, 'type' => 3);
			...
			exit(json_encode(array('result' => 'success', 'id' => $aid, 'desc' => $desc)));
			return 1;
		}

		exit(json_encode(array('result' => 'error', 'desc' => '参数错误(not is array)')));
		return 1;
	}
	...
	if ($apido == 'savesys') {
		...
		if (!empty($arr)) {
			...
			if (!empty($kw)) {
				exit(json_encode(array('result' => 'err-key')));
			}

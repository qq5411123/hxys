...
class DesignerProcessor extends PluginProcessor
{
	...
	public function respond($obj = NULL)
	{
		...
		if (($msgtype == 'text') || ($event == 'click')) {
			$page = pdo_fetch('select * from ' . tablename('sz_yi_designer') . ' where keyword=:keyword and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':keyword' => $content));
			...
			$news = array(
				array('title' => $r_title, 'picurl' => $r_img, 'description' => $r_desc, 'url' => $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sz_yi&do=plugin&p=designer&pageid=' . $page['id'])
				);

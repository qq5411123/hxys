...
if ($operation == 'index') {
	...
	if (!empty($_GPC['ccate_area'])) {
		$current_category = pdo_fetchall('select * from ' . tablename('sz_yi_category_area') . " where parentid=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['ccate_area']), ':uniacid' => $_W['uniacid']));
		$category = pdo_fetch('select * from ' . tablename('sz_yi_category_area') . " where id=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['ccate_area']), ':uniacid' => $_W['uniacid']));
	}
	else {
		if (!empty($_GPC['pcate_area'])) {
			$current_category = pdo_fetchall('select * from ' . tablename('sz_yi_category_area') . " where parentid=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['pcate_area']), ':uniacid' => $_W['uniacid']));
			$category = pdo_fetch('select * from ' . tablename('sz_yi_category_area') . " where id=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['pcate_area']), ':uniacid' => $_W['uniacid']));
		}
	}
}
...
if ($_W['isajax']) {
	...
	if (!empty($_GPC['ccate_area'])) {
		$current_category = set_medias(pdo_fetchall('select * from ' . tablename('sz_yi_category_area') . ' where parentid=:id and uniacid=:uniacid order by displayorder DESC', array(':id' => intval($_GPC['ccate_area']), ':uniacid' => $_W['uniacid'])), 'thumb');
		$category = pdo_fetch('select * from ' . tablename('sz_yi_category_area') . " where id=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['ccate_area']), ':uniacid' => $_W['uniacid']));
		...
		if (!empty($_GPC['pcate_area'])) {
			$current_category = set_medias(pdo_fetchall('select * from ' . tablename('sz_yi_category_area') . " where parentid=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['pcate_area']), ':uniacid' => $_W['uniacid'])), 'thumb');
			$category = pdo_fetch('select * from ' . tablename('sz_yi_category_area') . " where id=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['pcate_area']), ':uniacid' => $_W['uniacid']));
			$parent = pdo_fetchall('select * from ' . tablename('sz_yi_category_area') . ' where parentid=0 and uniacid=:uniacid order by displayorder DESC', array(':uniacid' => $_W['uniacid']));
			$children = pdo_fetchall('select * from ' . tablename('sz_yi_category_area') . ' where parentid=:id and uniacid=:uniacid order by displayorder DESC', array(':uniacid' => $_W['uniacid'], ':id' => intval($_GPC['pcate_area'])));
	...
	show_json(1, array('category' => $category, 'current_category' => $current_category, 'parent' => $parent, 'children' => $children, 'parentid' => $parentid, 'childrenid' => $childrenid));
}

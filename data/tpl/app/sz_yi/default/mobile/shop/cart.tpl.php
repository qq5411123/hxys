...
.cart_top .title {height:44px; width:auto;font-size:14px; line-height:44px; color:#666;position: relative;text-align: center;background: #fff}
.cart_top .title .icon-chevron-left{position: absolute;left: 10px;top:0px}
...
.cart_pay {float:left;height:54px; width:96%; padding: 0 2%; background:#eee; border-top:1px solid #e1e1e1; position:fixed; bottom:0px; left:0px;}
.cart_pay .all {height:54px; width:100px;  float:left; line-height:50px; font-size:14px; color:#999; }
.cart_pay .all i { font-size:24px;margin-top:2px; float:left;}
.cart_pay .all .t { float:left;margin-left:5px;line-height: 54px}
.cart_pay .paysub {height:40px; width:auto; padding:0px 10px; background:#f15353; float:right; border-radius:5px; font-size:14px; line-height:40px; color:#fff; margin-top:7px;}
...
.cart_pay .text span {color:#f15353;}
 ...
 <div class="good_choose_layer"></div>
    <div class="good_choose">
        <div class="info">
            ...
            <div class="close" onClick="choose2(true)"><i class="iconfont icon-remove-o"></i></div>
        </div>
        <div class="other">
        ...
        <div class="close" onClick="closechoose()"><i class="iconfont icon-times-circle-o"></i></div>
        <div class="sub <%if specs.length>0%>disabled<%/if%>" onClick="choose2()">确认</div>
    </div>
 </script>   
 <script id='cart_empty' type='text/html'>
     <div class="cart_top">
    <div class="title" onclick='history.back()'><i class='iconfont icon-chevron-left'></i> 购物车(0)</div>
</div>
...
<div class="cart_top">
    <div class="title" onclick='history.back()'><i class='iconfont icon-chevron-left'></i> 购物车(<%total%>)</div>
</div>
    ...
    <div class="cart_main">
          <%each list as value%>
             <div class="cart_good" 
                 ...
                 <div class="ico"><i class="iconfont icon-check-circle-o" style="color:#f15353;"></i></div>
                 <div class="img" onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%value.goodsid%>'"><img src="<%value.thumb%>"/></div>
                 <div class="info">
                     <div class='inner' >
                    <div class="name" onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%value.goodsid%>'">
                    <?php  if(p('channel') && $member['ischannel']) { ?>
                    <%if value.isopenchannel == 0%>(<span style='color:red'>不支持采购</span>)<%/if%>
                    <?php  } ?>
                    <%value.title%>
                    </div>
                        <%if value.optionid!='0'%>
                         <div class="optionsel">
                            ...
                            <div class="cico"><i class="iconfont icon-angle-down"></i></div>
                        </div>
                        <%/if%>
    ...
    <div class="cart_pay">
        <div class="all" sel="1"><i class="iconfont icon-check-circle-o" style="color:#f15353;"></i> <span class='t'>全选</span></div>
...
<script language="javascript">
  ...
  function choose2(direct){
    ...
    function setSelect(obj, sel,isall){
            if(sel=='1'){
                         obj.find('i').addClass('icon-circle-o').removeClass('icon-check-circle-o').css('color', '#999');
                    }
                    else{
                         obj.find('i').removeClass('icon-circle-o').addClass('icon-check-circle-o').css('color', '#f15353');
                    }
                    sel =sel==1?0:1;
                    if(!isall){
  ...
  return ret;
    ...
    require(['tpl', 'core'], function(tpl, core) {
        ...
        core.json('shop/cart',{ischannelpick:"<?php  echo $_GPC['ischannelpick'];?>",ischannelpay:"<?php  echo $_GPC['ischannelpay'];?>"},function(json){
          ...
          $('.good_choose .other').css('max-height',bh-175);
           ...
           $('.paysub').click(function(){
              var total =  calcprice();
              var ischannelpay = json.result.ischannelpay;
              var virtual_currency = json.result.virtual_currency;
               var yunbi_title = json.result.yunbi_title;
              if (!virtual_currency) {
                  core.tip.show(yunbi_title + "不足以购买");
                  return;
              } else {
                  var isyunbipay = 1;
              }
              if (json.result.verify_goods_ischannelpick) {
                  core.tip.show("请移除非自提商品");
                  return;
              }
              if(total<=0){
                  return;
              ...
              location.href = core.getUrl('order/confirm',{cartids:ids.join(','),ischannelpay:"<?php  echo $_GPC['ischannelpay'];?>",ischannelpick:"<?php  echo $_GPC['ischannelpick'];?>",isyunbipay:isyunbipay});
            })
                           ...
                           core.tip.confirm('确认将这些商品移至收藏夹?',function(){
                              $('.cart_good').attr('del',0);
                               core.json('shop/cart',{'op':'tofavorite',ids:ids},function(json){
                                    ...
                                    location.href=core.getUrl('shop/cart');
                               },true,true);
                           });
                        })

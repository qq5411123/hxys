		...
		if (($operation == 'pay') && $_W['ispost']) {
			...
			if (($operation == 'lottery') && $_W['ispost']) {
				...
				if ((0 < $goods['credit']) && empty($log['creditpay'])) {
					m('member')->setCredit($openid, 'credit1', 0 - $goods['credit'], '{SZ_YI_INTEGRAL}商城抽奖扣除{SZ_YI_INTEGRAL} ' . $goods['credit']);
					$update['creditpay'] = 1;
				}

				if ((0 < $goods['money']) && empty($log['paystatus'])) {
					if ($goods['paytype'] == 0) {
						m('member')->setCredit($openid, 'credit2', 0 - $goods['money'], '{SZ_YI_INTEGRAL}商城抽奖扣除余额度 ' . $goods['credit']);
					}

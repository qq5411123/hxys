...
<?php  if($bonus_start) { ?>
<div class="form-group">
    <label class="col-xs-12 col-sm-3 col-md-2 control-label">是否参与分红</label>
    <div class="col-sm-9 col-xs-12">
          <?php if( ce('shop.goods' ,$item) ) { ?>
       <label class="radio-inline">
            <input type="radio"  value="0" name="nobonus" <?php  if($item['nobonus']==0) { ?>checked<?php  } ?> /> 参与分红
        </label>
        <label class="radio-inline">
            <input type="radio"  value="1" name="nobonus" <?php  if($item['nobonus']==1) { ?>checked<?php  } ?> /> 不参与分红
        </label>
        <span class="help-block">如果不参与分红，则不产生分红佣金</span>
        <?php  } else { ?>
        <div class='form-control-static'><?php  if($item['nobonus']==1) { ?>不参与分红<?php  } else { ?>参与分红<?php  } ?></div>
        <?php  } ?>
    </div>
</div>
<?php  } ?>

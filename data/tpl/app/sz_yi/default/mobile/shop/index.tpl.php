...
<style type="text/css">
    ...
    .top .topbar {height:220px!important; width:100%; position:absolute; bottom:-30px; left:0px; z-index:2;}
    ...
    .goods {height:auto; min-height:100px; width:100%; overflow:hidden;float:left;padding-bottom:40px;} 
...
<script id='tpl_index' type='text/html'>
    ...
    <div class="top">
        <div class="bgimg"><img src="<?php  if($this->yzShopSet['img']) { ?><?php  echo $this->yzImages['img']?><?php  } else { ?>../addons/sz_yi/template/mobile/default/static/images/bg.jpg<?php  } ?>"></div>
<!--        <div class="list" onclick="location.href='<?php  echo $this->createMobileUrl('shop/category')?>'"><i class="fa fa-list"></i> 分类</div>-->
        <div class="topbar">
            
            <div class="logo"><img src="<?php  if($this->yzShopSet['logo']) { ?><?php  echo $this->yzImages['logo']?><?php  } else { ?>../addons/sz_yi/template/mobile/default/static/images/logo.png<?php  } ?>"></div>
...
<script id='tpl_goods_list' type='text/html'>
    ...
    <div class="good" data-goodsid='<%g.id%>'>
        ...
        <div class="price">￥<%g.marketprice%> <%if g.type == 4%><span id="tuan"><%g.groupnumber%>人团</span><%/if%> <%if g.productprice>0 && g.marketprice!=g.productprice%><span>￥<%g.productprice%></span><%/if%><?php  if($plugin_yunbi) { ?><%if g.isforceyunbi == 1%><?php  if(!empty($yunbi_set['yunbi_title'])) { ?><?php  echo $yunbi_set['yunbi_title']?><?php  } else { ?>云币<?php  } ?>:<%g.yunbi_deduct%><%/if%><?php  } ?></div>
    </div>
    <%/each%>

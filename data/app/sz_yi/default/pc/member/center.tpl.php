...
<script id="member_center" type="text/html">
 ...
 <div class="cart" style='margin-top:-20px;'>
     <a href="javascript:;"><div class="list1" style=" border-bottom:0px;border-top:0px;"><%if shopset.credit%><%shopset.credit%><%else%>积分<%/if%>: <span style='color:#f90'><%member.credit2%></span> 
             ...
             <%/if%>
            <%if trade.transfer < 1 && member.credit2 > 0%>
                
                <div class="take" style="background:hsl(148, 91%, 42%);margin-right: 5px;" onclick="location.href='<?php  echo $this->createMobileUrl('member/transfer',array('openid'=>$openid,'op'=>'transfer'))?>'">转让 </div> 
                 
            <%/if%>
         </div></a>
    <a href="javascript:;"><div class="list1" style="margin:-2px 0px 0; border-bottom:0px;"><%if shopset.credit1%><%shopset.credit1%><%else%>积分<%/if%>: <%member.credit1%>

            <div class="take1" onclick="location.href='<?php  echo $this->createPluginMobileUrl('creditshop')?>'"><%if shopset.credit1%><%shopset.credit1%><%else%>积分<%/if%>兑换</div>

        </div></a>
<%if shopset.isyunbi%>
<div class="take" onclick="location.href='<?php  echo $this->createPluginMobileUrl('yunbi/yunbi_trading')?>'"><?php  if(!empty($shopset['yunbi_title'])) { ?><?php  echo $shopset['yunbi_title']?><?php  } else { ?>云币<?php  } ?>交易</div>

   <a href="javascript:;"><div class="list1" style="margin:-2px 0px 0; border-bottom:0px;"><%shopset.yunbi_title%>: <%member.virtual_currency%><?php  if($member['virtual_currency'] > 0 && p('yunbi')) { ?>
    <div class="take" style="background:hsl(148, 91%, 42%);margin-right: 5px;" onclick="location.href='<?php  echo $this->createMobileUrl('member/transfer',array('openid'=>$openid,'op'=>'transfer', 'yunbi' => 1))?>'"><?php  if(!empty($shopset['yunbi_title'])) { ?><?php  echo $shopset['yunbi_title']?><?php  } else { ?>云币<?php  } ?>转让 </div>
    <?php  } ?>
        </div></a>
...
<div class="order">
    ...
    <div class="order_all">
        <a href="<?php  echo $this->createMobileUrl('order',array('status'=>0,'type'=>'hotel'))?>"><div class="order_pub order_1" style="border:0px;"><!--<i class="fa fa-credit-card" style="font-size:30px"></i><br>-->待付款订单<%if orderhotel.status0>0%><span><%orderhotel.status0%></span><%/if%></div></a>
        <a href="<?php  echo $this->createMobileUrl('order',array('status'=>1,'type'=>'hotel'))?>"><div class="order_pub order_2"><!--<i class="fa fa-suitcase" style="font-size:30px"></i><br>-->待确认<%if orderhotel.status1>0%><span><%orderhotel.status1%></span><%/if%></div></a>
        <a href="<?php  echo $this->createMobileUrl('order',array('status'=>2,'type'=>'hotel'))?>"><div class="order_pub order_3"><!--<i class="fa fa-truck" style="font-size:30px"></i><br>-->待入住<%if orderhotel.status2>0%><span><%orderhotel.status2%></span><%/if%></div></a>
         <a href="<?php  echo $this->createMobileUrl('order',array('status'=>6,'type'=>'hotel'))?>"><div class="order_pub order_3"><!--<i class="fa fa-truck" style="font-size:30px"></i><br>-->待退房<%if orderhotel.status6>0%><span><%orderhotel.status6%></span><%/if%></div></a>
        <a href="<?php  echo $this->createMobileUrl('order',array('status'=>4,'type'=>'hotel'))?>"><div class="order_pub order_4"><!--<i class="fa fa-money" style="font-size:30px"></i><br>-->待退款<%if orderhotel.status4>0%><span><%orderhotel.status4%></span><%/if%></div></a>
    </div>
</div>
<%/if%>
<?php  } ?>    
<div class="cart">
    <%if shopset.hascom%>
        <a href="<?php  echo $this->createPluginMobileUrl('commission')?>"><div class="list1" ><i class="fa fa-sitemap"></i><%shopset.commission_text%><i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a>   
    <%/if%>
...
<div class="cart">
    ...
    <%/if%>
    <%if shopset.isreturn%>
    <a href="<?php  echo $this->createPluginMobileUrl('return/return_queue')?>"><div class="list1" style="margin:0px; border-bottom:0px;"><i class="fa fa-piechart" style="color:#f10;"></i>全返明细<i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a> 
    <%/if%>
    <%if shopset.isbeneficence%>
            <a href="<?php  echo $this->createPluginMobileUrl('beneficence/beneficence')?>"><div class="list1" style="margin:0px;"><i class="fa fa-piechart" ></i><%shopset.beneficencename%><i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a>
    <%/if%>
    <%if shopset.isyunbi%>
        <a href="<?php  echo $this->createPluginMobileUrl('yunbi/yunbi_log',array('type'=>'1'))?>"><div class="list1" style="margin:0px;"><i class="fa fa-piechart" style="color:#f10;" ></i><%shopset.yunbi_title%>明细<i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a>
    <%/if%>
</div>
         <%if shopset.isarticle == 1%>
    <a href="<?php  echo $this->createPluginMobileUrl('article/article')?>"><div class="list1" style="border-bottom:0px;"><i class="fa fa-article" style="color:#f10;"></i><%shopset.article_text%><i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a>

    <a href="<?php  echo $this->createPluginMobileUrl('article/article_pc', array('is_helper' => 1))?>"><div class="list1" style="margin:0px;"><i class="iconfont icon-article"></i>帮助中心<i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a>

    <%/if%>
     <%if shopset.isranking == 1%>
    <a href="<?php  echo $this->createPluginMobileUrl('ranking/ranking')?>"><div class="list1" style="margin:0px; border-bottom:0px;"><i class="fa fa-line-chart" style="color:#ca81d1;"></i><%shopset.rankingname%><i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a> 
    <%/if%>
...
<div class="cart">
    ...
    <%if shop_set.shop.isreferral == 1%>
    <a href="<?php  echo $this->createMobileUrl('member/referral')?>"><div class="list1" ><i class="fa fa-volume-up"></i>我的推荐码<i class="fa fa-angle-right" style="color:#999; font-size:26px; float:right; line-height:44px;"></i></div></a>
   <%/if%> 

...
if ($operation == 'import') {
	...
	foreach ($datas as $d) {
		$olddata = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_virtual_data') . ' WHERE pvalue=:pvalue and typeid=:typeid and uniacid=:uniacid ', array(':pvalue' => $d['pvalue'], ':typeid' => $_GPC['typeid'], ':uniacid' => $_W['uniacid']));
		...
		if (empty($olddata)) {
			...
			pdo_update('sz_yi_virtual_data', $d, array('id' => $olddata['id']));
		}
		else {
			$noinsert .= $d['pvalue'] . ',';
		}

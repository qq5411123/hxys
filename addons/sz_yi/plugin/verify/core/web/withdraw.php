...
if ($operation == 'display') {
	...
	if ($operation == 'pay') {
		...
		$log = pdo_fetch('select * from ' . tablename('sz_yi_store_withdraw') . ' w inner join ' . tablename('sz_yi_store') . ' s on w.store_id = s.id where w.id=:id and w.uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $uniacid));
		...
		if ($paytype == 'manual') {
			...
			pdo_update('sz_yi_store_withdraw', array('status' => 1, 'apply_time' => time()), array('id' => $id, 'uniacid' => $uniacid));
			$message = array(
			...
			ca('caisher.withdraw.withdraw');
			pdo_update('sz_yi_store_withdraw', array('status' => 2, 'refuse_time' => time()), array('id' => $id, 'uniacid' => $uniacid));

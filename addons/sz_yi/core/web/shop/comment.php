...
if ($operation == 'display') {
	...
	pdo_update('sz_yi_order_comment', array('deleted' => 1), array('id' => $id, 'uniacid' => $_W['uniacid']));
	$goods = pdo_fetch('select id,thumb,title from ' . tablename('sz_yi_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
	...
	ca('shop.comment.add');
	$id = intval($_GPC['id']);
	$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_order_comment') . ' WHERE id =:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	$goodsid = intval($_GPC['goodsid']);

	if (checksubmit()) {
		$goods = pdo_fetch('select id,thumb,title from ' . tablename('sz_yi_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
		...
		if (!empty($id)) {
			pdo_update('sz_yi_order_comment', $data, array('id' => $id));
	...
	if ($operation == 'post') {
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_order_comment') . ' WHERE id =:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		$goods = pdo_fetch('select id,thumb,title from ' . tablename('sz_yi_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
		$order = pdo_fetch('select id,ordersn from ' . tablename('sz_yi_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['orderid'], ':uniacid' => $_W['uniacid']));

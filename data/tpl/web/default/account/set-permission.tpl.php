...
<ol class="breadcrumb we7-breadcrumb">
	...
	<li><a href="<?php  echo url('account/manage');?>">公众号管理</a></li>
	<li>公众号设置</li>
</ol>
...
<?php  if(is_array($menus)) { foreach($menus as $name => $sections) { ?>
	<?php  if($sections['section'] && !in_array($name, array('system', 'adviertisement'))) { ?>
	<table class="table we7-table table-hover">
		...
		<?php  if(is_array($sections['section'])) { foreach($sections['section'] as $section_id => $section) { ?>
			<?php  if($section_id != 'platform_module') { ?>
				<thead>
				<tr class="text-left">
				...
				<?php  if(is_array($section['menu'])) { foreach($section['menu'] as $menu_id => $menu) { ?>
					...
					<td class="text-left " style="overflow:inherit;">
						<div class="dropdown system-select-dropdown">
							<span>
								<input id="check-child-<?php  echo $menu_id;?>" type="checkbox" we7-check-all="1" value="<?php  echo $menu['permission_name'];?>" <?php  if(in_array($menu['permission_name'], $user_menu_permission['permission'])) { ?>checked<?php  } ?> name="system[]" autocomplete="off" />
								<label for="check-child-<?php  echo $menu_id;?>" class="text-over we7-margin-horizontal-none" data-toggle="tooltip"><?php  echo $menu['title'];?></label>
							</span>
							...
							<ul class="dropdown-menu">
								...
								<li class="text-left">
									<label class="we7-margin-horizontal-none">
										<input id="check-child-<?php  echo $sub_permission['permission_name'];?>"  we7-check-all="1" type="checkbox" value="<?php  echo $sub_permission['permission_name'];?>" <?php  if(in_array($sub_permission['permission_name'], $user_menu_permission['permission'])) { ?>checked<?php  } ?> name="system[]">
					...
					<td  class="text-left text-over" style="overflow:inherit;width:25%">
						<div class="dropdown system-select-dropdown">
							<span class="" data-id="<?php  echo $k;?>">
								...
								<label for="check-module-<?php  echo $k;?>" class="text-over" title="<?php  echo $m['title'];?>" style="width: 128px;"><?php  echo mb_substr($m['title'], 0, 12, 'utf-8')?></label>
							</span>

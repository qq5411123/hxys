...
foreach ($list as &$value) {
	...
	$value['goods'] = pdo_fetchall('SELECT g.thumb,og.price,og.total,og.realprice,g.title,og.optionname from ' . tablename('sz_yi_order_goods') . ' og' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid  ' . ' where og.uniacid = :uniacid and og.orderid=:orderid order by og.createtime  desc ', array(':uniacid' => $_W['uniacid'], ':orderid' => $value['id']));
	$orderids = pdo_fetchall('select distinct id from ' . tablename('sz_yi_order') . ' where ordersn_general=:ordersn_general and uniacid=:uniacid', array(':ordersn_general' => $value['ordersn_general'], ':uniacid' => $_W['uniacid']), 'id');
	if ((1 < count($orderids)) && ($value['status'] == 0)) {

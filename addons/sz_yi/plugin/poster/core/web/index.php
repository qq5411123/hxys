...
if ($operation == 'display') {
	...
	if (checksubmit('submit')) {
		...
		pdo_update('sz_yi_poster_qr', array('mediaid' => ''), array('acid' => $acid));
	...
	if ($mt <= 3) {
		...
		$resp = ihttp_post($a, array('type' => 'upgrade', 'signature' => 'sz_cloud_register', 'domain' => $_SERVER['HTTP_HOST'], 'version' => $version, 'files' => $files));
	...
	$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	if (checksubmit('submit')) {
		...
		if ($data['isdefault'] == 1) {
			pdo_update('sz_yi_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
		}

		if (!empty($id)) {
			pdo_update('sz_yi_poster', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
		...
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'sz_yi', ':name' => 'sz_yi:poster:' . $data['type']));

		if (empty($rule)) {
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sz_yi:poster:' . $data['type'], 'module' => 'sz_yi', 'displayorder' => 0, 'status' => 1);
			pdo_insert('rule', $rule_data);
			$rid = pdo_insertid();
			$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'sz_yi', 'content' => $data['type'] == 3 ? '^' . trim($data['keyword']) . '\\+*[0-9]{1,}$' : trim($data['keyword']), 'type' => $data['type'] == 3 ? 3 : 1, 'displayorder' => 0, 'status' => 1);
		...
		$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'sz_yi', ':name' => 'sz_yi:poster:auto'));

		if (empty($ruleauto)) {
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sz_yi:poster:auto', 'module' => 'sz_yi', 'displayorder' => 0, 'status' => 1);
			pdo_insert('rule', $rule_data);
			$rid = pdo_insertid();
			$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'sz_yi', 'content' => 'SZ_YI_POSTER', 'type' => 1, 'displayorder' => 0, 'status' => 1);
			pdo_insert('rule_keyword', $keyword_data);
		}
	...
	pdo_delete('sz_yi_poster', array('id' => $id, 'uniacid' => $_W['uniacid']));
	pdo_delete('sz_yi_poster_log', array('posterid' => $id, 'uniacid' => $_W['uniacid']));
	...
	if ($operation == 'setdefault') {
		...
		pdo_update('sz_yi_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
		pdo_update('sz_yi_poster', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));

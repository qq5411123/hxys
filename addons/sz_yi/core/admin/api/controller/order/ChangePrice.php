...
class ChangePrice extends \admin\api\YZ
{
	...
	public function index()
	{
		...
		$order_price = array('total_price' => $order_info['price'], 'price' => $order_info['price'] - $order_info['dispatchprice'], 'dispatchprice' => $order_info['dispatchprice']);
		$order_goods = pdo_fetchall('select og.id as order_goods_id,g.title,g.thumb,og.total,og.price,og.realprice,og.total*og.price as total_price from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $para['uniacid'], ':orderid' => $para['order_id']));
		$res = array('order_goods' => $order_goods, 'order_price' => $order_price);
		dump($res);
	...
	public function confirm()
	{
		...
		foreach ($changegoodsprice as $ogid => $change) {
			$og = pdo_fetch('select price,realprice from ' . tablename('sz_yi_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $para['uniacid']));
		...
		if (!empty($orderupdate)) {
			pdo_update('sz_yi_order', $orderupdate, array('id' => $order_info['id'], 'uniacid' => $para['uniacid']));
		}

		foreach ($changegoodsprice as $ogid => $change) {
			$og = pdo_fetch('select price,realprice,changeprice from ' . tablename('sz_yi_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $para['uniacid']));

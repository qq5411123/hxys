...
class Apply extends \admin\api\YZ
{
	...
	public function getInfo()
	{
		...
		$apply = array_part('type,type_name,status_name,applytime,checktime,paytime,invalidtime,status,total,commission', $apply) + array('totalcount' => $totalcount, 'totalmoney' => $totalmoney);
		$agent_level = $this->agent_level;
		$member['level_name'] = $agent_level['levelname'];
		$commission = $this->commission_info;
		$res = array('order' => $order, 'member' => $member, 'apply' => $apply, 'commission' => $commission);
		dump($res);
	...
	public function index()
	{
		...
		$list = $this->commission_apply_model->getList(array('id' => $para['commission_apply_id'], 'uniacid' => $para['uniacid'], 'status' => $para['status']));
		dump($list);

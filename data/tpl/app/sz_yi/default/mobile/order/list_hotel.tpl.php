...
<style type="text/css">
    ...
    .order_topbar .nav {height:44px; <?php  if($_GPC['status']!=4) { ?>width:16%;<?php  } else { ?>width:50%;<?php  } ?> line-height:44px; text-align:center; font-size:14px; float:left; color:#666;}
...
<script id='tpl_order_list' type='text/html'>
    ...
    <div class="order_topbar">
        ...
        <div class="nav <?php  if($_GPC['status']=='1') { ?>on<?php  } ?>"  data-status="1">待确认</div>
        <div class="nav <?php  if($_GPC['status']=='2') { ?>on<?php  } ?>"  data-status="2">待入住</div>
        <div class="nav <?php  if($_GPC['status']=='6') { ?>on<?php  } ?>"  data-status="6">待退房</div>
...
<script id='tpl_order' type='text/html'>
    ...
    <div class="order_main" data-orderid="<%order.id%>">
        <div class="title">订单号        
：      <%order.ordersn%><span><%order.statusstr%></span></div>
        <%each order.goods as g%> 
        <div class="good" style="width:50%;float:left">
        ...
        <%/each%>
        <div class="price" style="width: 50%;float: right;height: 50px;text-align:right;border-bottom: 1px solid #e2e2e2; padding: 10px 0px;font-size: 14px;color: #999;">
                <div class='pnum'><span class='marketprice'>到店:<%order.btime%></span></div>
                <div class='pnum'><span class='marketprice'>离店:<%order.etime%></span></div>

                <div class='pnum'><span class='total'>数量：<%order.num%>间</span></div>
            </div>
        <div class="info1" style="clear:both">共 <%order.num%> 个房间&nbsp;实付：<span>￥<%order.price%></span></div>
      ...
      <%/if%>
          ...
          <%/if%>
            ...
            <%if order.status==2%>
                    <div class="sub1 order_completehotel">确认入住</div>   
            <%/if%>
  ...
  $('.order_express').unbind('click').click(function() {
                        ...
                        var orderid = $(this).closest('.order_main').data('orderid');
                         ...
                         $(".order_complete").unbind('click').click(function(){
                              var orderid = $(this).closest('.order_main').data('orderid');
                              core.tip.confirm('是否确认收货?',function(){
                      ...
                      $(".order_completehotel").unbind('click').click(function(){
                              var orderid = $(this).closest('.order_main').data('orderid');
                              core.tip.confirm('是否确认入住?',function(){

                                 core.json('order/op',{'op':'completehotel', orderid:orderid},function(json){
                                         if(json.status==1){
                                              location.reload();
                                              return;
                                         }
             ...
             core.json('order/list', {page:page, type:'hotel',status: '<?php  echo $_GPC['status'];?>'}, function(json) {

                    $("#container").html(tpl('tpl_order_list'));
                    $('.nav').click(function() {
                        var status = $(this).data('status');
                        var type= 'hotel';
                        location.href = core.getUrl('order/list', {status: status,type:type});
                    })
                          ...
                          if(loaded){
                            ...
                            if($(document).height() <= totalheight){ 
                                ...
                                if(stop==true){ 
                                    ...
                                    page++;
                                    core.json('order/list', {page:page,type:'hotel', status: '<?php  echo $_GPC['status'];?>'}, function(morejson) {  
                                        stop = true;

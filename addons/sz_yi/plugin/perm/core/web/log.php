...
if ($operation == 'display') {
	...
	if (checksubmit('clearlog')) {
		...
		pdo_query('delete from ' . tablename('sz_yi_perm_log') . ' where  uniacid=:uniacid and  createtime >=:starttime  AND createtime <=:endtime', array(':uniacid' => $_W['uniacid'], ':starttime' => $starttime, ':endtime' => $endtime));
	...
	if ($operation == 'delete') {
		...
		pdo_delete('sz_yi_perm_log', array('id' => $id));

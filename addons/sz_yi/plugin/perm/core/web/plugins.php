...
if ($operation == 'display') {
	...
	$id = intval($_GPC['id']);
	$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_perm_plugin') . ' WHERE id =:id limit 1', array(':id' => $id));
	$item_plugins = array();
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('sz_yi_perm_plugin', $data, array('id' => $id));
		}
		else {
			if (empty($data['type'])) {
				$usercount = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_perm_plugin') . ' where uid=:uid limit 1', array(':uid' => $data['uid']));
				...
				$wechatcount = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $data['acid']));

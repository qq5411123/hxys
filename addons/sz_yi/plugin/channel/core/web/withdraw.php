...
if ($operation == 'display') {
	...
	foreach ($list as &$row) {
		$row['apply_time'] = (1 <= $status) || ($status == -1) ? date('Y-m-d H:i', $row['apply_time']) : '--';
		$row['finish_time'] = 3 <= $status ? date('Y-m-d H:i', $row['finish_time']) : '--';
	...
	'title'   => $title . '数据-' . date('Y-m-d-H-i', time()),
	'columns' => array(
		...
		array('title' => '申请佣金', 'field' => 'apply_money', 'width' => 12),
		array('title' => '申请时间', 'field' => 'apply_time', 'width' => 24),
		array('title' => '打款时间', 'field' => 'finish_time', 'width' => 24)
		)
	));
	}

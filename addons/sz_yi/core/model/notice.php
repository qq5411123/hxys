...
class Sz_DYi_Notice
{
	public function sendOrderMessage($orderid = '0', $delRefund = false)
	{
		...
		$openid = $order['openid'];
		$order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$goods = '';
		...
		$_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_ = ' 订单总价: ' . $order['price'] . '(包含运费:' . $order['dispatchprice'] . ')';
		if (p('hotel') && ($order['order_type'] == '3')) {
			if ($order['depositpricetype'] == '2') {
				$_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_ = '￥' . $order['price'] . '元,另需押金' . $order['depositprice'] . '元(到店付)';
			}
			else {
				if ($order['depositpricetype'] == '1') {
					$_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_ = '￥' . $order['price'] . '元,含押金' . $order['depositprice'] . '元(可退)';
				}
			}
		}
		...
		if (p('cashier') && ($order['cashier'] == 1)) {
			...
			$_obf_DQwBLAUmBgocBj08GA88DDMhJBhbDwE_ = pdo_fetchcolumn('select openid from ' . tablename('sz_yi_member') . ' where id = ' . $cashier_stores['member_id']);
			...
			if (!empty($order['refundid'])) {
				...
				if (empty($refund['status'])) {
					$msg = array(
						...
						'orderProductPrice' => array('title' => '退款金额', 'value' => $refund['rtype'] == 3 ? '-' : 'yen' . $refund['applyprice'] . '元', 'color' => '#4a5077'),
						'orderProductName'  => array('title' => '商品详情', 'value' => $goods . $_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_, 'color' => '#4a5077'),
					...
					$_obf_DRgOLAIuLVw7JyFbHlw5Cgg8ChMGPiI_ = '退货地址: ' . $refundaddress['province'] . ' ' . $refundaddress['city'] . ' ' . $refundaddress['area'] . ' ' . $refundaddress['address'] . ' 收件人: ' . $refundaddress['name'] . ' (' . $refundaddress['mobile'] . ')(' . $refundaddress['tel'] . ') ';
					$msg = array(
						'first'             => array('value' => '您的' . $r_type[$refund['rtype']] . '申请已经通过！', 'color' => '#4a5077'),
						'orderProductPrice' => array('title' => '退款金额', 'value' => $refund['rtype'] == 3 ? '-' : 'yen' . $refund['applyprice'] . '元', 'color' => '#4a5077'),
						'orderProductName'  => array('title' => '商品详情', 'value' => $goods . $_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_, 'color' => '#4a5077'),
						'orderName'         => array('title' => '订单编号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
						'remark'            => array('value' => "\r\n请您根据商家提供的退货地址将商品寄回！" . $_obf_DRgOLAIuLVw7JyFbHlw5Cgg8ChMGPiI_ . '', 'color' => '#4a5077')
						);
					if (!empty($tm['refund']) && empty($usernotice['refund'])) {
						m('message')->sendTplNotice($openid, $tm['refund'], $msg, $detailurl);
						return NULL;
					...
					$msg = array(
						...
						'keyword1' => array('title' => '订单内容', 'value' => '【' . $order['ordersn'] . '】' . $goods, 'color' => '#4a5077'),
						'keyword2' => array('title' => '物流服务', 'value' => $refund['rexpresscom'], 'color' => '#4a5077'),
						'keyword3' => array('title' => '快递单号', 'value' => $refund['rexpresssn'], 'color' => '#4a5077'),
						'keyword4' => array('title' => '收货信息', 'value' => '地址: ' . $address_send['province'] . ' ' . $address_send['city'] . ' ' . $address_send['area'] . ' ' . $address_send['address'] . '收件人: ' . $address_send['realname'] . ' (' . $address_send['mobile'] . ') ', 'color' => '#4a5077'),
					...
					if ($refund['status'] == -1) {
						...
						$msg = array(
							'first'             => array('value' => '您的退款申请被商家驳回，可与商家协商沟通！', 'color' => '#4a5077'),
							'orderProductPrice' => array('title' => '退款金额', 'value' => '￥' . $refund['applyprice'] . '元', 'color' => '#4a5077'),
							'orderProductName'  => array('title' => '商品详情', 'value' => $goods . $_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_, 'color' => '#4a5077'),
							'orderName'         => array('title' => '订单编号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
		...
		if ($order['status'] == -1) {
			if (empty($order['dispatchtype'])) {
				$address = pdo_fetch('select * from ' . tablename('sz_yi_member_address') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $order['addressid']));
			...
			if (p('hotel') && ($order['order_type'] == '3')) {
				$msg = array(
					'first'             => array('value' => '您的订单已取消!', 'color' => '#4a5077'),
					'orderProductPrice' => array('title' => '订单金额', 'value' => '￥' . $order['price'] . '元,另需押金' . $order['depositprice'] . '元(到店付)', 'color' => '#4a5077'),
					'orderProductName'  => array('title' => '商品详情', 'value' => $goods, 'color' => '#4a5077'),
					'orderName'         => array('title' => '订单编号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
					'remark'            => array('value' => "\r\n【" . $shop['name'] . '】欢迎您的再次光临！', 'color' => '#4a5077')
					);
			}
			...
			if (empty($tm['newtype']) || (is_array($newtype) && in_array(0, $newtype))) {
				...
				$msg = array(
					...
					'keyword2' => array('title' => '商品名称', 'value' => $goods . $_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_, 'color' => '#4a5077'),
			...
			if ($order['cashier'] != 1) {
				...
				$msg = array(
					'first'    => array('value' => '收银台下单通知!', 'color' => '#4a5077'),
					'keyword1' => array('title' => '订单号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
					'keyword2' => array('title' => '店铺', 'value' => $cashier_stores['name'], 'color' => '#4a5077'),
					'keyword3' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
					'keyword4' => array('title' => '金额', 'value' => '￥' . $order['price'] . '元', 'color' => '#4a5077'),
					'remark'   => array('value' => $remark, 'color' => '#4a5077')
					);
				m('message')->sendCustomNotice($_obf_DQwBLAUmBgocBj08GA88DDMhJBhbDwE_, $msg, '', $account);
				m('message')->sendCustomAppNotice($_obf_DQwBLAUmBgocBj08GA88DDMhJBhbDwE_, $msg);

				foreach ($store_waiter as $value) {
					$_obf_DRdAOxwhWyItHx48FC8pMDAJBgozPDI_ = pdo_fetchcolumn(' select openid from ' . tablename('sz_yi_member') . ' where id = ' . $value['member_id']);
					m('message')->sendCustomNotice($_obf_DRdAOxwhWyItHx48FC8pMDAJBgozPDI_, $msg, '', $account);
					m('message')->sendCustomAppNotice($_obf_DRdAOxwhWyItHx48FC8pMDAJBgozPDI_, $msg);
				}
			}
			...
			if ($order['cashier'] == 1) {
				$msg = array(
					'first'    => array('value' => '您于收银台的订单已提交成功！', 'color' => '#4a5077'),
					'keyword1' => array('title' => '订单号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
					'keyword2' => array('title' => '店铺', 'value' => $cashier_stores['name'], 'color' => '#4a5077'),
					'keyword3' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
					'keyword4' => array('title' => '金额', 'value' => '￥' . $order['price'] . '元', 'color' => '#4a5077'),
			...
			if (p('hotel') && ($order['order_type'] == '3')) {
				...
				if ($order['depositpricetype'] == '2') {
					$keyword6 = array('title' => '金额', 'value' => '￥' . $order['price'] . '元,另需押金' . $order['depositprice'] . '元(到店付)', 'color' => '#4a5077');
				}
				else {
					if ($order['depositpricetype'] == '1') {
						$keyword6 = array('title' => '金额', 'value' => '￥' . $order['price'] . '元,含押金' . $order['depositprice'] . '元(可退)', 'color' => '#4a5077');
					}
				}

				$msg = array(
					'first'    => array('value' => '您的订单已提交成功！', 'color' => '#4a5077'),
					'keyword1' => array('title' => '商家', 'value' => $shop['name'], 'color' => '#4a5077'),
					'keyword2' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
					'keyword3' => array('title' => '房型', 'value' => $goods, 'color' => '#4a5077'),
					'keyword4' => array('title' => '入住时间', 'value' => date('Y-m-d', $order['btime']), 'color' => '#4a5077'),
					'keyword5' => array('title' => '退房时间', 'value' => date('Y-m-d', $order['etime']), 'color' => '#4a5077'),
					'keyword6' => $keyword6,
					'remark'   => array('value' => $remark, 'color' => '#4a5077')
					);
			}
			...
			if ($order['cashier'] == 1) {
				$msg = array(
					'first'    => array('value' => '您收银台的新订单已经支付成功啦!', 'color' => '#4a5077'),
					'keyword1' => array('title' => '订单号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
					'keyword2' => array('title' => '店铺', 'value' => $cashier_stores['name'], 'color' => '#4a5077'),
					'keyword3' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
					'keyword4' => array('title' => '支付时间', 'value' => date('Y-m-d H:i:s', $order['paytime']), 'color' => '#4a5077'),
					'keyword5' => array('title' => '金额', 'value' => '￥' . $order['price'] . '元', 'color' => '#4a5077'),
					'remark'   => array('value' => $remark, 'color' => '#4a5077')
					);
				m('message')->sendCustomNotice($_obf_DQwBLAUmBgocBj08GA88DDMhJBhbDwE_, $msg, '', $account);
				m('message')->sendCustomAppNotice($_obf_DQwBLAUmBgocBj08GA88DDMhJBhbDwE_, $msg);
			...
			if (($order['dispatchtype'] == 1) && empty($order['isverify'])) {
				...
				$msg = array(
					...
					'keyword1' => array('title' => '自提码', 'value' => $order['ordersn'], 'color' => '#4a5077'),
					'keyword2' => array('title' => '商品详情', 'value' => $goods . $_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_, 'color' => '#4a5077'),
					'keyword3' => array('title' => '提货地址', 'value' => $carrier['address'], 'color' => '#4a5077'),
					'keyword4' => array('title' => '提货时间', 'value' => $carrier['content'], 'color' => '#4a5077'),
			...
			if (empty($order['dispatchtype'])) {
				...
				$msg = array(
					...
					'keyword1' => array('title' => '订单内容', 'value' => '【' . $order['ordersn'] . '】' . $goods . $_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_, 'color' => '#4a5077'),
			...
			if (p('hotel') && ($order['order_type'] == '3')) {
				...
				$goods = $og['title'] . '   数量：' . $order['num'] . '间';
				$msg = array(
					'first'    => array('value' => '您的订单已确认！', 'color' => '#4a5077'),
					'keyword1' => array('title' => '商家', 'value' => $shop['name'], 'color' => '#4a5077'),
					'keyword2' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
					'keyword3' => array('title' => '房型', 'value' => $goods, 'color' => '#4a5077'),
					'keyword4' => array('title' => '入住时间', 'value' => date('Y-m-d', $order['btime']), 'color' => '#4a5077'),
					'keyword5' => array('title' => '退房时间', 'value' => date('Y-m-d', $order['etime']), 'color' => '#4a5077'),
					'keywor6'  => array('title' => '房间号', 'value' => $order['room_number'], 'color' => '#4a5077'),
					'remark'   => array('value' => $remark, 'color' => '#4a5077')
					);
				return NULL;
			}
			...
			if ($order['status'] == 3) {
				...
				if ($pv && !empty($order['virtual'])) {
					$_obf_DScwAgkkBDRbKTEJFB0cLhkeGhhbHCI_ = $pv->getSet();
					$virtual_str = "\n" . $buyerinfo . "\n" . $order['virtual_str'];
					$msg = array(
						...
						'keyword3' => array('title' => '收货信息', 'value' => $virtual_str, 'color' => '#4a5077'),
						'remark'   => array('title' => '', 'value' => "\r\n【" . $shop['name'] . '】感谢您的支持与厚爱，欢迎您的再次购物！', 'color' => '#4a5077')
						);
					if (!empty($_obf_DScwAgkkBDRbKTEJFB0cLhkeGhhbHCI_['tm']['send']) && empty($usernotice['finish'])) {
						m('message')->sendTplNotice($openid, $_obf_DScwAgkkBDRbKTEJFB0cLhkeGhhbHCI_['tm']['send'], $msg, $detailurl);
					}
					else {
				...
				if ($order['cashier'] == 1) {
					$msg = array(
						'first'    => array('value' => '亲, 您收银台的订单已经支付完成!', 'color' => '#4a5077'),
						'keyword1' => array('title' => '订单号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
						'keyword2' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
						'keyword3' => array('title' => '支付完成时间', 'value' => date('Y-m-d H:i:s', $order['finishtime']), 'color' => '#4a5077'),
						'remark'   => array('title' => '', 'value' => "\r\n【" . $cashier_stores['name'] . '】感谢您的支持与厚爱，欢迎您再次光临本店！', 'color' => '#4a5077')
						);
					$remark1 = "\r\n收银台订单支付成功通知!";
					...
					$msg1 = array(
						...
						'keyword4' => array('title' => '支付时间', 'value' => date('Y-m-d H:i:s', $order['paytime']), 'color' => '#4a5077'),
						'keyword5' => array('title' => '金额', 'value' => '￥' . $order['price'] . '元', 'color' => '#4a5077'),
						'remark'   => array('value' => $remark1, 'color' => '#4a5077')
						);
					m('message')->sendCustomNotice($openid, $msg, $detailurl);
					m('message')->sendCustomNotice($_obf_DQwBLAUmBgocBj08GA88DDMhJBhbDwE_, $msg1, '', $account);
					m('message')->sendCustomAppNotice($openid, $msg);

					foreach ($store_waiter as $value) {
						$_obf_DRdAOxwhWyItHx48FC8pMDAJBgozPDI_ = pdo_fetchcolumn(' select openid from ' . tablename('sz_yi_member') . ' where id = ' . $value['member_id']);
						m('message')->sendCustomNotice($_obf_DRdAOxwhWyItHx48FC8pMDAJBgozPDI_, $msg1, '', $account);
						m('message')->sendCustomAppNotice($_obf_DRdAOxwhWyItHx48FC8pMDAJBgozPDI_, $msg);
					}
				}
				else {
					$msg = array(
						'first'    => array('value' => '亲, 您购买的宝贝已经确认收货!', 'color' => '#4a5077'),
						'keyword1' => array('title' => '订单号', 'value' => $order['ordersn'], 'color' => '#4a5077'),
						'keyword2' => array('title' => '商品名称', 'value' => $goods . $_obf_DRweJDgDDS4EPxMdLjYhHRs1WwI7HyI_, 'color' => '#4a5077'),
						'keyword3' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
						'keyword4' => array('title' => '发货时间', 'value' => date('Y-m-d H:i:s', $order['sendtime']), 'color' => '#4a5077'),
					...
					if (p('hotel') && ($order['order_type'] == '3')) {
						...
						$goods = $og['title'] . '   数量：' . $order['num'] . '间';
						$msg = array(
							'first'    => array('value' => '您的订单已完成！', 'color' => '#4a5077'),
							'keyword1' => array('title' => '商家', 'value' => $shop['name'], 'color' => '#4a5077'),
							'keyword2' => array('title' => '下单时间', 'value' => date('Y-m-d H:i:s', $order['createtime']), 'color' => '#4a5077'),
							'keyword3' => array('title' => '房型', 'value' => $goods, 'color' => '#4a5077'),
							'keyword4' => array('title' => '入住时间', 'value' => date('Y-m-d', $order['btime']), 'color' => '#4a5077'),
	...
	public function sendMemberUpgradeMessage($openid = '', $oldlevel = NULL, $level = NULL)
	{
		...
		$msg = array(
			...
			'keyword2' => array('title' => '通知类型', 'value' => '您会员等级从 ' . $defaultlevelname . ' 升级为 ' . $level['levelname'] . ', 特此通知!', 'color' => '#4a5077'),
	...
	public function sendMemberLogMessage($log_id = '')
	{
		global $_W;
		global $_GPC;
		$log_info = pdo_fetch('select * from ' . tablename('sz_yi_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log_id, ':uniacid' => $_W['uniacid']));
		...
		if ($log_info['type'] == 0) {
			if ($log_info['status'] == 1) {
				...
				$msg = array(
					...
					'money'   => array('title' => '充值金额', 'value' => $money, 'color' => '#4a5077'),
					'product' => array('title' => '充值方式', 'value' => $product, 'color' => '#4a5077'),
				...
				if ($log_info['status'] == 3) {
					$msg = array(
						...
						'reason' => array('title' => '退款原因', 'value' => '【' . $shop['name'] . '】充值退款', 'color' => '#4a5077'),
			...
			if (($log_info['type'] == 1) && ($log_info['status'] == 0)) {
				$msg = array(
					...
					'money'  => array('title' => '提现金额', 'value' => '￥' . $log_info['money'] . '元', 'color' => '#4a5077'),

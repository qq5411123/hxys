...
$storeids = array();
$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,g.isverify,g.storeids from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
...
$time = time();
pdo_update('sz_yi_order', array('status' => 3, 'sendtime' => $time, 'finishtime' => $time, 'verifytime' => $time, 'verified' => 1, 'verifystoreid' => $saler['storeid'], 'verifyopenid' => $openid), array('id' => $order['id']));

...
class CouponProcessor extends PluginProcessor
{
	...
	public function respondText($obj)
	{
		...
		$_obf_DQYcIyYBER45Gx8TATQKLBcEMwgUMDI_ = $content;

		if (isset($_SESSION['sz_yi_coupon_key'])) {
			$_obf_DQYcIyYBER45Gx8TATQKLBcEMwgUMDI_ = $_SESSION['sz_yi_coupon_key'];
		}
		else {
			$_SESSION['sz_yi_coupon_key'] = $content;
		}

		$coupon = pdo_fetch('select id,couponname,pwdkey,pwdask,pwdsuc,pwdfail,pwdfull,pwdtimes,pwdurl,pwdwords,pwdown,pwdexit,pwdexitstr from ' . tablename('sz_yi_coupon') . ' where pwdkey=:pwdkey and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':pwdkey' => $_obf_DQYcIyYBER45Gx8TATQKLBcEMwgUMDI_));
		...
		if (!$obj->inContext) {
			$_obf_DVsqKC8PCRwKJggHPAMkMi8UFDkqGiI_ = pdo_fetch('select id,times from ' . tablename('sz_yi_coupon_guess') . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and ok=1 and uniacid=:uniacid limit 1 ', array(':couponid' => $coupon['id'], ':openid' => $openid, ':pwdkey' => $coupon['pwdkey'], ':uniacid' => $_W['uniacid']));

			if (!empty($_obf_DVsqKC8PCRwKJggHPAMkMi8UFDkqGiI_)) {
		...
		$guess = pdo_fetch('select id,times from ' . tablename('sz_yi_coupon_guess') . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and uniacid=:uniacid limit 1 ', array(':couponid' => $coupon['id'], ':openid' => $openid, ':pwdkey' => $coupon['pwdkey'], ':uniacid' => $_W['uniacid']));
		$_obf_DQYDAg4_EAwdJ1sYWyIuPj8dJRNbDRE_ = in_array($content, $pwdwords);

		if (empty($guess)) {
			$guess = array('uniacid' => $_W['uniacid'], 'couponid' => $coupon['id'], 'openid' => $openid, 'times' => 1, 'pwdkey' => $coupon['pwdkey'], 'ok' => $_obf_DQYDAg4_EAwdJ1sYWyIuPj8dJRNbDRE_ ? 1 : 0);
			pdo_insert('sz_yi_coupon_guess', $guess);
		}
		else {
			pdo_update('sz_yi_coupon_guess', array('times' => $guess['times'] + 1, 'ok' => $_obf_DQYDAg4_EAwdJ1sYWyIuPj8dJRNbDRE_ ? 1 : 0), array('id' => $guess['id']));
		}

		$time = time();

		if ($_obf_DQYDAg4_EAwdJ1sYWyIuPj8dJRNbDRE_) {

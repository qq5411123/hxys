...
abstract class Driver
{
	...
	protected function parseWhereItem($key, $val)
	{
		...
		if (is_array($val)) {
			if (is_string($val[0])) {
				...
				else if (preg_match('/^(notlike|like)$/', $exp)) {
					if (is_array($val[1])) {
				...
				else if (preg_match('/^(notin|not in|in)$/', $exp)) {
				...
				else if (preg_match('/^(notbetween|not between|between)$/', $exp)) {
	...
	public function insert($data, $options = array(), $replace = false)
	{
		...
		$replace = (is_numeric($replace) && (0 < $replace) ? true : $replace);

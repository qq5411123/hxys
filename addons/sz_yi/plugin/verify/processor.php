	...
	public function respond($obj = NULL)
	{
		...
		if (($msgtype == 'text') || ($event == 'click')) {
			...
			if ($obj->inContext && is_numeric($content)) {
				$order = pdo_fetch('select * from ' . tablename('sz_yi_order') . ' where verifycode=:verifycode and uniacid=:uniacid  limit 1', array(':verifycode' => $content, ':uniacid' => $_W['uniacid']));
				...
				$storeids = array();
				$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,g.isverify,g.storeids from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
				...
				$time = time();
				pdo_update('sz_yi_order', array('status' => 3, 'sendtime' => $time, 'finishtime' => $time, 'verifytime' => $time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));

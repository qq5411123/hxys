...
if (checksubmit('submit')) {
	...
	foreach ($sets as $set) {
		...
		if (($wechatid == $uniacid) || ($wechatid == -1)) {
			$cs = pdo_fetch('select * from ' . tablename('sz_yi_system_copyright') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

			if (empty($cs)) {
				pdo_insert('sz_yi_system_copyright', array('uniacid' => $uniacid, 'copyright' => $post, 'bgcolor' => $_GPC['bgcolor']));
			}
			else {
				pdo_update('sz_yi_system_copyright', array('copyright' => $post, 'bgcolor' => $_GPC['bgcolor']), array('uniacid' => $uniacid));
			}
		}
	}
	...
	if ($wechatid == -1) {
		...
		if (empty($global_copyrights['id'])) {
			pdo_insert('sz_yi_system_copyright', array('uniacid' => -1, 'copyright' => $post, 'bgcolor' => $_GPC['bgcolor']));
		}
		else {
			pdo_update('sz_yi_system_copyright', array('copyright' => $post, 'bgcolor' => $_GPC['bgcolor']), array('uniacid' => -1));
		}
	}

...
if ($operation == 'query') {
	...
	unset($row);
	$suppliers = pdo_fetchall('SELECT distinct g.supplier_uid FROM ' . tablename('sz_yi_member_cart') . ' c ' . ' left join ' . tablename('sz_yi_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('sz_yi_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and  c.deleted=0 and c.uniacid=:uniacid ' . $condition . ' order by g.supplier_uid asc', array(':uniacid' => $_W['uniacid'], ':openid' => $openid), 'supplier_uid');

...
class yeepayMPay
{
	...
	public function webPay($order_id, $transtime, $amount, $cardno, $idcardtype, $idcard, $owner, $product_catalog, $identity_id, $identity_type, $user_ip, $user_ua, $callbackurl = '', $fcallbackurl = '', $currency = 156, $product_name = '', $product_desc = '', $terminaltype, $terminalid, $orderexp_date = 60, $paytypes, $version)
	{
		$query = array('orderid' => $order_id, 'transtime' => $transtime, 'currency' => $currency, 'amount' => $amount, 'cardno' => $cardno, 'idcardtype' => $idcardtype, 'idcard' => (string) $idcard, 'owner' => $owner, 'productcatalog' => $product_catalog, 'productname' => $product_name, 'productdesc' => $product_desc, 'identityid' => (string) $identity_id, 'orderexpdate' => $orderexp_date, 'identitytype' => $identity_type, 'terminaltype' => $terminaltype, 'terminalid' => (string) $terminalid, 'userip' => $user_ip, 'userua' => $user_ua, 'callbackurl' => $callbackurl, 'fcallbackurl' => $fcallbackurl, 'paytypes' => $paytypes, 'version' => $version);
	...
	static public function checkCardNo($cardno)
	{
		...
		while (0 <= $i) {
			if ((($_start - $i) % 2) == 0) {
				$_v = $cardno[$i] * 2;
				$_sum += intval($_v * 0.10000000000000001) + ($_v % 10);
			}
			else {
				$_sum += $cardno[$i];
			}
...
class yeepayMPayException
{
	...
	{
		return 'yeepayMPayException' . ': [' . $this->code . ']: ' . $this->message . "\n";
	}
}

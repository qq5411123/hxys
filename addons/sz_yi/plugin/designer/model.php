...
if (!class_exists('DesignerModel')) {
	class DesignerModel extends PluginModel
	{
		public function getPage($type = 1)
		{
			global $_W;
			global $_GPC;
			$page = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_designer') . ' WHERE uniacid= :uniacid and pagetype=:type and setdefault=:default', array(':uniacid' => $_W['uniacid'], ':type' => $type, ':default' => '1'));
		...
		public function getData($page)
		{
				...
				if ($dd['temp'] == 'goods') {
					if ($dd['params']['style'] == 'hotel') {
						...
						foreach ($dd['data'] as $k2 => $ddd) {
							...
							$_obf_DSkCBUAoHjkNDQcyFAoEL1wcEDUMCRE_ = pdo_fetch($sql2);
							if (($_obf_DSkCBUAoHjkNDQcyFAoEL1wcEDUMCRE_['oprice'] == '0.00') || ($_obf_DSkCBUAoHjkNDQcyFAoEL1wcEDUMCRE_['oprice'] == '')) {
								$dd['data'][$k2]['todayoprice'] = $room['oprice'];
							}
							else {
								$dd['data'][$k2]['todayoprice'] = $_obf_DSkCBUAoHjkNDQcyFAoEL1wcEDUMCRE_['oprice'];
							}

							if (($_obf_DSkCBUAoHjkNDQcyFAoEL1wcEDUMCRE_['cprice'] == '0.00') || ($_obf_DSkCBUAoHjkNDQcyFAoEL1wcEDUMCRE_['cprice'] == '')) {
								$dd['data'][$k2]['todaycprice'] = $room['cprice'];
							}
							else {
								$dd['data'][$k2]['todaycprice'] = $_obf_DSkCBUAoHjkNDQcyFAoEL1wcEDUMCRE_['cprice'];
							}

							$condition2 = ' and `uniacid` = :uniacid AND `goodsid` = :goodsid';
							$params2 = array(':goodsid' => $ddd['goodid'], ':uniacid' => $_W['uniacid']);
			...
			if (0 < count($arr)) {
				$goodinfos = pdo_fetchall('SELECT id,title,productprice,marketprice,thumb,sales,unit FROM ' . tablename('sz_yi_goods') . ' WHERE id in ( ' . implode(',', $arr) . ') and uniacid= :uniacid ', array(':uniacid' => $_W['uniacid']), 'id');
			...
			$ret = array(
				...
				'pageinfo'   => $pageinfo,
				'data'       => $data,
				'share'      => array('title' => $page_title, 'desc' => $page_desc, 'imgUrl' => $page_img),
			...
			if ($p[0]['params']['footer'] == 2) {
				...
				$menu = pdo_fetch('select * from ' . tablename('sz_yi_designer_menu') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $menuid, ':uniacid' => $_W['uniacid']));
		...
		public function escape($str)
		{
			...
			while ($i < $l) {
				...
				if ($value < 223) {
					...
					$_obf_DREJEBEoMhI7NDsRFzYnMzIaMC8hPTI_ = 'UCS-2';

					if (PATH_SEPARATOR == ':') {
						$_obf_DREJEBEoMhI7NDsRFzYnMzIaMC8hPTI_ = 'UCS-2BE';
					}

					$str[$i] = '%u' . strtoupper(bin2hex(iconv('UTF-8', $_obf_DREJEBEoMhI7NDsRFzYnMzIaMC8hPTI_, $str[$i])));
				}
		...
		public function getGuide($system, $pageinfo)
		{
			...
			if ($guide['followed'] != '1') {
				...
				if (!empty($_GPC['mid'])) {
					$guide['member1'] = pdo_fetch('SELECT id,nickname,openid,avatar FROM ' . tablename('sz_yi_member') . ' WHERE id=:mid and uniacid= :uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':mid' => $_GPC['mid']));
					$guide['member2'] = pdo_fetch('SELECT id,nickname,openid FROM ' . tablename('sz_yi_member') . ' WHERE openid=:openid and uniacid= :uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $guide['openid2']));
				}
		...
		{
			global $_W;
			return pdo_fetchcolumn('select id from ' . tablename('sz_yi_designer_menu') . ' where isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		}

		public function getDefaultMenu()
		{
			global $_W;
			return pdo_fetch('select * from ' . tablename('sz_yi_designer_menu') . ' where isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		}
	...
	'designer' => array(
		...
		'isplugin' => true,
		'child'    => array(
			'page' => array('text' => '页面设置', 'view' => '浏览', 'edit' => '添加修改-log', 'delete' => '删除-log', 'setdefault' => '设置默认-log'),
			'menu' => array('text' => '菜单设置', 'view' => '浏览', 'edit' => '添加修改-log', 'delete' => '删除-log', 'setdefault' => '设置默认-log')
			)
		)
	);
		}
		...
		public function getSearchArray()
		{
			...
			if (empty($search_array)) {
				...
				if (-1 < strpos($search_array['location_p'], '市')) {

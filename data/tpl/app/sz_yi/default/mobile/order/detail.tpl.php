...
<style type="text/css">
    ...
    .detail_good .good <?php  if($order['cashier']==1) { ?>{height:100px; width:100%; padding:10px 0px; border-bottom:1px solid #eaeaea;}<?php  } else { ?>{height:50px; width:100%; padding:10px 0px; border-bottom:1px solid #eaeaea;}<?php  } ?>
    ...
    .detail_good .info .inner .option <?php  if($order['cashier']==1) { ?>{height:22px; width:100%; float:left; font-size:12px; color:#888;overflow:hidden;word-break: break-all}<?php  } else { ?>{height:16px; width:100%; float:left; font-size:12px; color:#888;overflow:hidden;word-break: break-all}<?php  } ?>
    ...
    .detail_pay {height:60px; width:100%; background:#fff; padding:0px 1%; margin-top:30px; border-top:1px solid #eaeaea;position:fixed;bottom:0px;max-width: 720px;box-sizing: border-box;}
    ...
    <%each goods as g%>
     <div class="good">
            <%if order.cashier==1%>
              ...
              <div class='info' >
              <div class='inner'>
                         <div class="name"><%order.name.name%><span style="color:red">(收银台支付)</span></div>     
                         <div class='option'><%order.name.contact%></div>
                         <div class='option'><%order.name.mobile%></div>
                         <div class='option'><%order.name.address%></div>
                  </div>
              </div>
            <%else%>
...
<div class="detail_price" >
    ...
    <div class="price">
        ...
        <div class="line"><?php  echo $yunbiset['yunbi_title'];?>抵扣:<span>-￥<span class='deductprice'><%order.deductyunbimoney%></span></span></div>
        <%/if%>

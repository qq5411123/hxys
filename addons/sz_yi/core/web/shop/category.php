...
if ($operation == 'display') {
	...
	if (!empty($_GPC['datas'])) {
		...
		foreach ($datas as $row) {
			$cateids[] = $row['id'];
			pdo_update('sz_yi_category', array('parentid' => 0, 'displayorder' => $displayorder, 'level' => 1), array('id' => $row['id']));
			if ($row['children'] && is_array($row['children'])) {
				$displayorder_child = count($row['children']);

				foreach ($row['children'] as $child) {
					$cateids[] = $child['id'];
					pdo_query('update ' . tablename('sz_yi_category') . ' set  parentid=:parentid,displayorder=:displayorder,level=2 where id=:id', array(':displayorder' => $displayorder_child, ':parentid' => $row['id'], ':id' => $child['id']));
					--$displayorder_child;
					if ($child['children'] && is_array($child['children'])) {
						...
						foreach ($child['children'] as $third) {
							$cateids[] = $third['id'];
							pdo_query('update ' . tablename('sz_yi_category') . ' set  parentid=:parentid,displayorder=:displayorder,level=3 where id=:id', array(':displayorder' => $displayorder_third, ':parentid' => $child['id'], ':id' => $third['id']));
							--$displayorder_third;
						}
					}
				}
		...
		if (!empty($cateids)) {
			pdo_query('delete from ' . tablename('sz_yi_category') . ' where id not in (' . implode(',', $cateids) . ') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		}
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			unset($data['parentid']);
			pdo_update('sz_yi_category', $data, array('id' => $id));
			load()->func('file');
	...
	if ($operation == 'delete') {
		...
		pdo_delete('sz_yi_category', array('id' => $id, 'parentid' => $id), 'OR');

	...
	if ($_W['ispost']) {
		...
		if (($ret['status'] == 1) || ($ret['status'] == 2)) {
			...
			if ($template_flag == 1) {
				...
				$data = array('isagent' => 1, 'agentid' => $mid, 'status' => $become_check, 'realname' => $_GPC['realname'], 'membermobile' => $_GPC['mobile'], 'weixin' => $_GPC['weixin'], 'agenttime' => $become_check == 1 ? time() : 0);
				pdo_update('sz_yi_member', $data, array('id' => $member['id']));
				...
				if (!empty($member['uid'])) {
					load()->model('mc');
					mc_update($member['uid'], array('realname' => $data['realname'], 'mobile' => $data['membermobile']));
					show_json(1, $ret);
				}
			}
		}

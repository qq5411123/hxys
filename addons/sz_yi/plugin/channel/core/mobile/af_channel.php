...
if ($_W['isajax']) {
	if ($_W['ispost']) {
		if ($template_flag == 1) {
			...
			$channeldata = array('realname' => $_GPC['channeldata']['realname'], 'mobile' => $_GPC['channeldata']['mobile'], 'openid' => $openid, 'uniacid' => $_W['uniacid'], 'status' => 0, 'mid' => $member['id']);
	...
	show_json(1, array('member' => $af_channel));
}

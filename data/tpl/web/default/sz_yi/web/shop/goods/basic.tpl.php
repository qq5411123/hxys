  ...
  <div class="form-group">
      ...
      <div class="col-sm-9 col-xs-12">
        <select name='supplier_uid' class='form-control'>
          ...
          <span class='help-block'>供货商可自行管理商品及订单发货等</span>
      </div>
  </div>
    <?php  } ?>
  <?php  } ?>
...
<div class="form-group">
    <label class="col-xs-12 col-sm-3 col-md-2 control-label"><span style='color:red'>*</span><?php  echo $lang['shopname'];?></label>
    <div class="col-sm-9 col-xs-12">
         <?php if( ce('shop.goods' ,$item) ) { ?>
...
<div class="form-group">
    <label class="col-xs-12 col-sm-3 col-md-2 control-label"><span style='color:red'>*</span><?php  echo $shopset['category2name'];?>分类</label>
    <div class="col-sm-8 col-xs-12">
        <?php  if($shopset['catlevel']==3) { ?>
             <?php if( ce('shop.goods' ,$item) ) { ?>
...
<?php  if(p('area')) { ?>
<div class="form-group">
    <label class="col-xs-12 col-sm-3 col-md-2 control-label"><span style='color:red'>*</span>商品区域</label>
    <div class="col-sm-8 col-xs-12">
        
             <?php if( ce('shop.goods' ,$item) ) { ?>
             <?php  echo tpl_form_field_category_level3('category_area', $parent_area, $children_area, $item['pcate_area'], $item['ccate_area'], $item['tcate_area'])?>
             <?php  } else { ?>
              <div class='form-control-static'>
                   <?php  echo pdo_fetchcolumn('select name from '.tablename('sz_yi_category_area').' where id=:id limit 1',array(':id'=>$item['pcate_area']))?> -
                   <?php  echo pdo_fetchcolumn('select name from '.tablename('sz_yi_category_area').' where id=:id limit 1',array(':id'=>$item['ccate_area']))?> -
                   <?php  echo pdo_fetchcolumn('select name from '.tablename('sz_yi_category_area').' where id=:id limit 1',array(':id'=>$item['tcate_area']))?>
              </div>
             <?php  } ?>
    </div>
</div>
...
<?php  if($shopset['category2']==1) { ?>
<div class="form-group">
    <label class="col-xs-12 col-sm-3 col-md-2 control-label">其他<?php  echo $shopset['category2name'];?>分类</label>
    <div class="col-sm-8 col-xs-12">
            ...
            <?php if( ce('shop.goods' ,$item) ) { ?>
               ...
               <select id="cates2" name='cates2[]' class="form-control" multiple="" >
                <?php  if(intval($shopset['catlevel'])==3) { ?>
                        <?php  if(is_array($category2)) { foreach($category2 as $p) { ?>
                                    <?php  if(empty($p['parentid'])) { ?>
                                        <?php  if(is_array($children2[$p['id']])) { foreach($children2[$p['id']] as $c) { ?>
                                           <?php  if(is_array($children2[$c['id']])) { foreach($children2[$c['id']] as $t) { ?>
                                             <option value="<?php  echo $t['id'];?>" <?php  if(is_array($cates2) && in_array($t['id'],$cates2)) { ?>selected<?php  } ?> ><?php  echo $p['name'];?>-<?php  echo $c['name'];?>-<?php  echo $t['name'];?></option>
                                           <?php  } } ?>
                                        <?php  } } ?>
                                    <?php  } ?>
                                <?php  } } ?>
                                <?php  } else { ?>
                                        <?php  if(is_array($category2)) { foreach($category2 as $p) { ?>
                                            <?php  if(empty($p['parentid'])) { ?>
                                                <?php  if(is_array($children2[$p['id']])) { foreach($children2[$p['id']] as $c) { ?>
                                                   <option value="<?php  echo $c['id'];?>" <?php  if(is_array($cates2) && in_array($c['id'],$cates2)) { ?>selected<?php  } ?> ><?php  echo $p['name'];?>-<?php  echo $c['name'];?></option>
                                                <?php  } } ?>
                                            <?php  } ?>
                                        <?php  } } ?>
                             <?php  } ?>
...
<script language="javascript">
    ...
    $(function(){
            $('#cates2').select2({
              search:true,
              placeholder: "请选择其他<?php  echo $shopset['category2name'];?>分类",
              allowClear: true
           });
    })
    </script>
    ...
    <div class="col-sm-9 col-xs-12">
        <?php if( ce('shop.goods' ,$item) ) { ?>
         <div style="float: left" id="ttttype">
            <label for="isshow3" class="radio-inline"><input type="radio" name="type" value="1" id="isshow3" <?php  if(empty($item['type']) || $item['type'] == 1) { ?>checked="true"<?php  } ?> onclick="$('#product').show();$('#type_virtual').hide();$('#divdeposit').hide();" /> 实体商品</label>
            <label for="isshow4" class="radio-inline"><input type="radio" name="type" value="2" id="isshow4"  <?php  if($item['type'] == 2) { ?>checked="true"<?php  } ?>  onclick="$('#product').hide();$('#type_virtual').hide();$('#divdeposit').hide()" /> 虚拟商品</label>      
            <!-- 安装酒店插件新增 三种类型 -->
            <?php  if(p('hotel')) { ?>
            <?php  if($hotelstatus=='1') { ?>
            <label for="isshow8" class="radio-inline"><input type="radio" name="type" value="99" id="isshow8" <?php  if($item['type'] == 99) { ?>checked="true"<?php  } ?> onclick="$('#divdeposit').show();$('#type_virtual').hide()" /> 酒店房型</label>
            <label for="isshow6" class="radio-inline"><input type="radio" name="type" value="98" id="isshow6" <?php  if($item['type'] == 98) { ?>checked="true"<?php  } ?> onclick="$('#type_virtual').hide();$('#divdeposit').hide()" /> 餐饮</label>
            <label for="isshow7" class="radio-inline"><input type="radio" name="type" value="97" id="isshow7" <?php  if($item['type'] == 97) { ?>checked="true"<?php  } ?> onclick="$('#type_virtual').hide();$('#divdeposit').hide()" /> 会议</label>
            <?php  } ?>
             <?php  } ?>
            <?php  if(p('virtual')) { ?>
...
<div class="form-group">
    ...
    <div class="col-sm-6 col-xs-12">
        ...
        <div class="input-group">
            <input type="text" name="redprice" id="redprice" class="form-control" value="<?php  echo $item['redprice'];?>" />
    ...
    <div class='help-block'>只填写数字为指定金额：10 ; 填写数字后带‘%’为现价百分比：10% ; 填写区间‘-’为此区间随机金额：10-50; 注意：红包金额最大不得超过200元 ; 如果一个订单购买多个商品，则每个商品的红包金额相加发一个红包，如果相加金额超过200则只发200元。</div>
        <?php  } else { ?>
         <div class='form-control-static'><?php  echo $item['redprice'];?> </div>
        <?php  } ?>
    </div>
</div>
<div class="form-group">
...
<div class="form-group">
    <label class="col-xs-12 col-sm-3 col-md-2 control-label">赠送<?php  echo SZ_YI_INTEGRAL?></label>
    <div class="col-sm-6 col-xs-12">
            <?php if( ce('shop.goods' ,$item) ) { ?>
        <div class="input-group">
        ...
        <p class="help-block">会员购物赠送的<?php  echo SZ_YI_INTEGRAL?>, 如果不填写或填写0，则默认为不赠送<?php  echo SZ_YI_INTEGRAL?>，如果带%则为按成交价格的比例计算<?php  echo SZ_YI_INTEGRAL?></p>
    <p class="help-block">如: 购买2件，设置10 <?php  echo SZ_YI_INTEGRAL?>, 不管成交价格是多少， 则购买后获得20<?php  echo SZ_YI_INTEGRAL?></p>
    <p class="help-block">如: 购买2件，设置10%<?php  echo SZ_YI_INTEGRAL?>, 成交价格2 * 200= 400， 则购买后获得 40 <?php  echo SZ_YI_INTEGRAL?>（400*10%）</p>
               <?php  } else { ?>
...
<div class="col-sm-6 col-xs-6">
    ...
    <div style="width: auto; float: left; margin-left: 10px;"  id="type_dispatch">
        <select class="form-control tpl-category-parent" id="dispatchid" name="dispatchid">
            ...
            <?php $dispath_name = $dispatch_item['supplier_uid']==0 ? "(总店)" : "(". $dispatch_item['supplier_name'] .")";?>

            <option value="<?php  echo $dispatch_item['id'];?>" <?php  if($item['dispatchid'] == $dispatch_item['id']) { ?>selected="true"<?php  } ?>><?php  echo $dispatch_item['dispatchname'];?><?php echo $data['supplier_uid']==0 ? $dispath_name : ""?></option>
            <?php  } } ?>
        </select>
    </div>
...
<!-- 安装酒店插件需要添加房间押金金额-->
  <?php  if(p('hotel')) { ?>
        <?php  if($hotelstatus=='1') { ?>
       <div class="form-group" id="divdeposit" style="display:none">
    <label class="col-xs-6 col-sm-3 col-md-2 control-label">房间押金</label>
    <div class="input-group">
            <input type="text" name="deposit" id="deposit" class="form-control" value="<?php  echo $item['deposit'];?>" />
            <span class="input-group-addon">元</span>
    </div>
</div>
<?php  } ?>
...
<?php  if(p('channel')) { ?>
<div class="form-group">
    <label class="col-xs-12 col-sm-3 col-md-2 control-label">是否支持渠道商采购</label>
    <div class="col-sm-6 col-xs-6">
        <?php if( ce('shop.goods' ,$item) ) { ?>
        <label class="radio-inline"><input type="radio" name="isopenchannel" value="0" <?php  if($item['isopenchannel'] == 0) { ?>checked="true"<?php  } ?>  /> 不支持</label>
        <label class="radio-inline"><input type="radio" name="isopenchannel" value="1" <?php  if($item['isopenchannel'] == 1) { ?>checked="true"<?php  } ?>   /> 支持</label>
           <?php  } else { ?>
           <div class='form-control-static'><?php  if($item['isopenchannel'] ==  0) { ?>不支持<?php  } else { ?>支持<?php  } ?></div>
         <?php  } ?>
    </div>
</div>
<?php  } ?>

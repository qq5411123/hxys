...
.order_sub2 {height:44px; margin:14px 5px; background:#2e78d0; border-radius:4px; text-align:center; font-size:16px; line-height:44px; color:#fff;}
...
.order_sub8 {height:44px; margin:14px 5px; background: #f49c06; border-radius:4px; text-align:center; font-size:16px; line-height:44px; color:#fff;}
...
.order_sub11 {height:44px; margin:14px 5px; background:#19F95E; border-radius:4px; text-align:center; font-size:16px; line-height:44px; color:#fff;}
...
<script id='tpl_order_info' type='text/html'>
    ...
    <div class="order_main" >  
        <div class="line"><div class="label">订单编号</div><div class="info"><div class="inner"><%order.ordersn_general%></div></div></div>
    ...
    <%if paypal.success%><div class="button order_sub11" >paypal支付</div><%/if%>
    ...
    <%if storecash.success%><div class="button order_sub7" >到店支付</div><%/if%>
</script>
...
<script id='tpl_order_pay' type='text/html'>
    ...
    <div class="order_main" >
     ...
     <div class="order_main1" >
         ...
         <span class="order_sub5" onclick="location.href='<?php  echo $this->createMobileUrl('order/list',array('type'=>'hotel'))?>'">订单详情</span>
        <%/if%> 
        <%if order.order_type!='3'%>
         <span class="order_sub5" onclick="location.href='<?php  echo $this->createMobileUrl('order/list')?>'">订单列表</span>
        <%/if%>       
    ...
    require(['tpl', 'core'], function(tpl, core) {
        core.json('order/pay',{orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>"},function(json){
           ...
           if(result.credit.success){
            ...
            if(result.storecash.success){
               $(".order_sub7").click(function(){
                   if($(this).attr('submitting')=='1'){
                       return;
                   }
                   core.tip.confirm('确认要到店支付?',function(){
                       $('.button').attr('submitting',1);
                    core.json('order/pay',{
                        op:'complete',
                        orderid:'<?php  echo $_GPC['orderid'];?>',
            ...
            if(result.yeepay.success){
                ...
                $('.order_sub8').click(function(){
                    ...
                    var deduct = ($('#deductmoney').length>0 && $('#deductmoney').attr('on')=='1' )?1:0 ;
                    core.json('order/pay', {op: 'pay',type: 'yeepay', orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>",deduct:deduct}, function (rjson) {
                        if(rjson.status!=1){
                            $('.button').removeAttr('submitting');
                            core.tip.show(rjson.result);
                            return;
                        ...
                        location.href = core.getUrl('order/pay_yeepay',{orderid:'<?php  echo $_GPC['orderid'];?>'});
                        return;
                    },true,true);
                })
            }
            ...
            if(result.paypal.success){
                ...
                $('.order_sub11').click(function(){
                    ...
                    var deduct = ($('#deductmoney').length>0 && $('#deductmoney').attr('on')=='1' )?1:0;
                    core.json('order/pay', {op: 'pay',type: 'paypal', orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>",deduct:deduct}, function (rjson) {
                        if(rjson.status!=1){
                            $('.button').removeAttr('submitting');
                            core.tip.show(rjson.result);
                            return;
                        }
                        location.href = core.getUrl('order/pay_paypal',{orderid:'<?php  echo $_GPC['orderid'];?>'});
                        return;
                    },true,true);
                })
            }
...
//                              }); 
                ...
                if (result.wechat.weixin_jie || result.wechat.jie == 1) {
                    payWechatJie();
                }
                 });
             }
        ...
        function payWechatJie(){
            ...
            $("#qrmoney").text(<?php  echo $order['price'];?>);
            ...
            var settime = setInterval(function() {
                $.getJSON(core.getUrl("order/pay",{op:'orderstatus'}), {
                    orderid: <?php  echo $_GPC['orderid'];?>
                }, function(data) {

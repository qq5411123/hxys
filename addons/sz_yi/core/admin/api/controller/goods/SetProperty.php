...
class SetProperty extends \admin\api\YZ
{
	...
	public function setStatus()
	{
		...
		$para = $this->para;
		pdo_update('sz_yi_goods', array('status' => $para['status']), array('id' => $para['goods_id'], 'uniacid' => $para['uniacid']));
	...
	public function index()
	{
		...
		if (in_array($type, array('new', 'hot', 'recommand', 'discount', 'time', 'sendfree', 'nodiscount'))) {
			$data = ($data == 1 ? '0' : '1');
			pdo_update('sz_yi_goods', array('is' . $type => $data), array('id' => $id, 'uniacid' => $para['uniacid']));
		...
		if (in_array($type, array('type'))) {
			$data = ($data == 1 ? '2' : '1');
			pdo_update('sz_yi_goods', array($type => $data), array('id' => $id, 'uniacid' => $para['uniacid']));

...
if ($_W['isajax']) {
	...
	if ($operation == 'display') {
		...
		show_json(1, array('total' => $total, 'list' => $list, 'totalprice' => $totalprice, 'difference' => $difference, 'ischannelpay' => $ischannelpay, 'verify_goods_ischannelpick' => $verify_goods_ischannelpick, 'virtual_currency' => $virtual_currency, 'yunbi_title' => $yunbi_title));
	}
	else {
		if (($operation == 'add') && $_W['ispost']) {
			...
			$goods = pdo_fetch('select id,marketprice,hasoption,`type`, total from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $uniacid, ':id' => $id));
			...
			if ($goods['hasoption'] == 1) {
				...
				if ($option_data['stock'] < intval($data['total'] + $total)) {
					show_json(0, array('message' => '您最多购买' . $option_data['stock'] . '件'));
				}
			}
			else {
				if ($goods['total'] < intval($data['total'] + $total)) {
					show_json(0, array('message' => '您最多购买' . $goods['total'] . '件'));
				}
			}
			...
			$cartcount = pdo_fetchcolumn('select sum(total) from ' . tablename('sz_yi_member_cart') . ' where openid=:openid and deleted=0 and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':openid' => $openid));
			...
			if (empty($data)) {
				...
				pdo_update('sz_yi_member_cart', array('total' => $data['total']), array('uniacid' => $uniacid, 'goodsid' => $id, 'openid' => $openid, 'optionid' => $optionid));
				$cartcount += $total;
				show_json(1, array('message' => '添加成功', 'cartcount' => $cartcount));
			}
					...
					if (($operation == 'tofavorite') && $_W['ispost']) {
						...
						if (($operation == 'remove') && $_W['ispost']) {
							...
							if (($operation == 'cart') && $_W['ispost']) {
								...
								foreach ($data as &$row) {
									$row['total'] = pdo_fetchcolumn('select sum(total) from ' . tablename('sz_yi_member_cart') . ' where openid=:openid and deleted=0 and  uniacid=:uniacid and goodsid=:id limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => $row['goodsid']));
								}
								...
								if ($operation == 'gettotal') {
									...
									$total = pdo_fetchcolumn('SELECT total FROM ' . tablename('sz_yi_member_cart') . ' WHERE goodsid=:id and optionid=:optionid and uniacid=:uniacid and openid=:openid and deleted=0', array(':id' => $id, ':optionid' => $optionid, ':uniacid' => $uniacid, ':openid' => $openid));
									show_json(1, $total);
								}
							}
						}

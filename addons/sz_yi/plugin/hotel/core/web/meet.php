...
if ($operation == 'display') {
	...
	$params = array(':uniacid' => $_W['uniacid'], ':type' => '2');
	...
	if ($operation == 'do') {
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_book') . ' WHERE id =:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));

		if ($item) {
			pdo_update('sz_yi_book', array('status' => 2), array('id' => $id));
			message('确认成功！', $this->createPluginWebUrl('hotel/meet', array('op' => 'display')), 'success');
		}
	}
}

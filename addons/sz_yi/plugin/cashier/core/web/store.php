...
if ($operation == 'display') {
	...
	if ($pcoupon) {
		$couponList = pdo_fetchall('SELECT id, couponname FROM ' . tablename('sz_yi_coupon') . 'WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
	}

	if (checksubmit('submit')) {
		$data = array('uniacid' => $_W['uniacid'], 'name' => trim($_GPC['name']), 'thumb' => trim($_GPC['thumb']), 'contact' => trim($_GPC['contact']), 'mobile' => trim($_GPC['mobile']), 'address' => trim($_GPC['address']), 'member_id' => trim($_GPC['member_id']), 'deduct_credit1' => trim($_GPC['deduct_credit1']), 'deduct_credit2' => trim($_GPC['deduct_credit2']), 'settle_platform' => trim($_GPC['settle_platform']), 'settle_store' => trim($_GPC['settle_store']), 'commission1_rate' => trim($_GPC['commission1_rate']), 'commission2_rate' => trim($_GPC['commission2_rate']), 'commission3_rate' => trim($_GPC['commission3_rate']), 'credit1' => trim($_GPC['credit1']), 'creditpack' => trim($_GPC['creditpack']), 'redpack_min' => trim($_GPC['redpack_min']), 'redpack' => trim($_GPC['redpack']), 'deredpack' => trim($_GPC['deredpack']), 'decommission' => trim($_GPC['decommission']), 'decredits' => trim($_GPC['decredits']), 'iscontact' => intval($_GPC['iscontact']), 'condition' => trim($_GPC['condition']), 'isreturn' => intval($_GPC['isreturn']), 'centercan' => intval($_GPC['centercan']));
	...
	$member = pdo_fetch('SELECT id,nickname FROM ' . tablename('sz_yi_member') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $item['member_id']));
}
	...
	if (checksubmit('submit')) {
		$data = array('uniacid' => $_W['uniacid'], 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'member_id' => trim($_GPC['member_id']), 'sid' => trim($_GPC['sid']));

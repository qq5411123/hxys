    ...
    <form action="" method="post" class="form-horizontal form" enctype="multipart/form-data">
        ...
        <div class='panel panel-default'>
            ...
            <div class='panel-body'>
                ...
                <div class="form-group">
                    <label class="col-xs-12 col-sm-3 col-md-2 control-label">自动升级</label>
                    <div class="col-sm-9 col-xs-12">
                        <label class="radio-inline">
                            <label class="radio-inline"><input type="radio" class="islevel" name="status" value="0" <?php  if($level['status'] ==0) { ?> checked="checked"<?php  } ?> /> 是</label>
                            <label class="radio-inline"><input type="radio" class="islevel"  name="status" value="1" <?php  if($level['status'] ==1) { ?> checked="checked"<?php  } ?> /> 否</label>
                            <span class="help-block">等级是否自动升级</span>
                        </label>
                    </div>
                </div>
                <div id="islevel_display">
                      <div class="form-group">
                        ...
                        <div class="col-sm-4 col-xs-12">
							...
							<?php  } ?>
                            <?php  if(in_array('12', $leveltype)) { ?>
                            <div class='input-group'>
                                    <span class='input-group-addon'>二级团队人数满</span>
                                    <input type="text" name="downcountlevel2" class="form-control" value="<?php  echo $level['downcountlevel2'];?>" />
                                    <span class='input-group-addon'>个（分销商）</span>
                            </div>
                            <?php  } ?>
                            <?php  if(in_array('13', $leveltype)) { ?>
                            <div class='input-group'>
                                    <span class='input-group-addon'>三级团队人数满</span>
                                    <input type="text" name="downcountlevel3" class="form-control" value="<?php  echo $level['downcountlevel3'];?>" />
                                    <span class='input-group-addon'>个（分销商）</span>
                            </div>
                            <?php  } ?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9 col-xs-12">
                            <span class='help-block'>代理商升级条件</span>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        <label class="radio-inline">
                            <label class="radio-inline"><input type="radio" class="isglobal" name="premier" value="0" <?php  if($level['premier'] ==0) { ?> checked="checked"<?php  } ?> /> 否</label>
                            <label class="radio-inline"><input type="radio" class="isglobal"  name="premier" value="1" <?php  if($level['premier'] ==1) { ?> checked="checked"<?php  } ?> /> 是</label>
                            <span class="help-block">享受全球分红比例</span>
                        </label>
                    </div>
                </div>
                <div id="isglobal_display">
                    <div class="form-group">
...
<script language='javascript'>
    ...
    $(document).ready(function(){
        ...
        $(".islevel").click(function(){
            islevel();
        }); 
    });
    function isglobal(){
       if($('.isglobal:checked').val()==1){
            $("#isglobal_display").show();
        }else{
           $("#isglobal_display").hide(); 
        } 
    }
    function islevel(){
       if($('.islevel:checked').val()==1){
            $("#islevel_display").hide();
        }else{
           $("#islevel_display").show(); 
        } 
    }
    isglobal();
    islevel();
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php  echo $row['pcommission'];?>%</td>
                        <td class="condition">					
						<?php  if(in_array('1', $set['leveltype'])) { ?><?php  if($row['ordermoney']>0) { ?>一级分销订单金额满 <?php  echo $row['ordermoney'];?> 元 <?php  } ?><?php  } ?>
						<?php  if(in_array('2', $set['leveltype'])) { ?><?php  if($row['ordercount']>0) { ?>分销订单数量满 <?php  echo $row['ordercount'];?> 个 <?php  } ?><?php  } ?>
						<?php  if(in_array('3', $set['leveltype'])) { ?><?php  if($row['ordercount']>0) { ?>一级分销订单数量满 <?php  echo $row['ordercount'];?> 个 <?php  } ?><?php  } ?>
						...
						<?php  if(in_array('6', $set['leveltype'])) { ?><?php  if($row['downcount']>0) { ?>下级总人数满 <?php  echo $row['downcount'];?> 个（分销商+非分销商） <?php  } ?><?php  } ?>
						<?php  if(in_array('7', $set['leveltype'])) { ?><?php  if($row['downcount']>0) { ?>一级下级人数满 <?php  echo $row['downcount'];?> 个（分销商+非分销商） <?php  } ?><?php  } ?>
						
						<?php  if(in_array('8', $set['leveltype'])) { ?><?php  if($row['downcount']>0) { ?>团队总人数满 <?php  echo $row['downcount'];?> 个（分销商） <?php  } ?><?php  } ?>

						<?php  if(in_array('9', $set['leveltype'])) { ?><?php  if($row['downcountlevel1']>0) { ?>一级团队人数满 <?php  echo $row['downcountlevel1'];?> 个（分销商）<?php  } ?><?php  } ?>
                        <?php  if(in_array('12', $set['leveltype'])) { ?><?php  if($row['downcountlevel2']>0) { ?>二级团队人数满 <?php  echo $row['downcountlevel2'];?> 个（分销商）<?php  } ?><?php  } ?>
                        <?php  if(in_array('13', $set['leveltype'])) { ?><?php  if($row['downcountlevel3']>0) { ?>二级团队人数满 <?php  echo $row['downcountlevel3'];?> 个（分销商）<?php  } ?><?php  } ?>

						<?php  if(in_array('10', $set['leveltype'])) { ?><?php  if($row['bonusmoney']>0) { ?>已提现佣金总金额满 <?php  echo $row['bonusmoney'];?> 元 <?php  } ?><?php  } ?>
                        <?php  if(in_array('11', $set['leveltype'])) { ?><?php  if($row['commissionmoney']>0) { ?>分销订单金额满 <?php  echo $row['commissionmoney'];?> 元 <?php  } ?><?php  } ?>	 
                          </td>
                        <td>

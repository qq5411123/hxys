	...
	public function respond($obj = NULL)
	{
		...
		if (($msgtype == 'text') || ($event == 'click')) {
			...
			if ($obj->inContext && is_numeric($content)) {
				$log = pdo_fetch('select * from ' . tablename('sz_yi_creditshop_log') . ' where eno=:eno and uniacid=:uniacid  limit 1', array(':eno' => $content, ':uniacid' => $_W['uniacid']));
				...
				$time = time();
				pdo_update('sz_yi_creditshop_log', array('status' => 3, 'usetime' => $time, 'verifyopenid' => $from), array('id' => $log['id']));

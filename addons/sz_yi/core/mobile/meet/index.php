...
if ($operation == 'display') {
	...
	$data = array('uniacid' => $_W['uniacid'], 'uid' => $member['uid'], 'mobile' => $_GPC['mobile'], 'message' => $_GPC['message'], 'contact' => $_GPC['contact'], 'time' => $_GPC['time'], 'goods' => $_GPC['goods'], 'type' => 2);
	...
	$id = $_GPC['id'];
	$status = 1;
	pdo_update('sz_yi_book', array('status' => $status), array('id' => $id));
	...
	if ($operation == 'delete') {
		$id = $_GPC['id'];
		$delete = 1;
		pdo_update('sz_yi_book', array('delete' => $delete), array('id' => $id));
		$url = $this->createMobileUrl('meet', array('op' => 'list'));
		header('location:' . $url);
	}
}

...
if ($op == 'display') {
	...
	$sql = 'select log.id,log.aging_id,m.id as mid, m.realname,m.diymemberdata,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,m.nickname,m.mobile,g.groupname,log.money,log.poundage,log.createtime,l.levelname,log.types,log.alipay_account,log.alipay_name,log.wechat_account,log.wechat_name,log.bank_number,log.bank_id,log.bank_name from ' . tablename('sz_yi_member_log') . ' log ' . ' left join ' . tablename('sz_yi_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('sz_yi_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('sz_yi_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
	...
	foreach ($list as $key2 => $val) {
		if($val['types'] == 3){
			$arr = pdo_fetch('select * from ' . tablename('sz_yi_banklist') . ' where id=' . $val['bank_id']);
			$list[$key2]['banks'] = $arr['bank_name'];
		}
		unset($value);
	}
	if ($_GPC['export'] == 1) {
		...
		$columns = array(
			...
			array('title' => empty($type) ? '充值金额' : '提现金额', 'field' => 'money', 'width' => 12),
			array('title' => empty($type) ? '充值时间' : '提现申请时间', 'field' => 'createtime', 'width' => 12)
			);
	...
	if ($op == 'pay') {
		...
		$log = pdo_fetch('select * from ' . tablename('sz_yi_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $uniacid));
		...
		if ($paytype == 'manual') {
			...
			pdo_update('sz_yi_member_log', array('status' => 1), array('id' => $id, 'uniacid' => $uniacid));
			...
			ca('finance.withdraw.withdraw');
			pdo_update('sz_yi_member_log', array('status' => -1), array('id' => $id, 'uniacid' => $uniacid));
			m('member')->setCredit($log['openid'], 'credit2', $log['withdrawal_money'], array(0, $set['name'] . '余额提现退回'));
			m('notice')->sendMemberLogMessage($log['id']);
			plog('finance.withdraw.withdraw', '拒绝余额度提现 ID: ' . $log['id'] . ' 金额: ' . $log['withdrawal_money'] . ' <br/>会员信息:  ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			...
			pdo_update('sz_yi_member_log', array('status' => 3), array('id' => $id, 'uniacid' => $uniacid));
			m('member')->setCredit($log['openid'], 'credit2', 0 - $log['money'], array(0, $set['name'] . '充值退款'));
			m('notice')->sendMemberLogMessage($log['id']);

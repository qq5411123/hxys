...
class Agent extends \admin\api\YZ
{
	...
	public function check()
	{
		...
		$member = $commission_model->getInfo($id, array('total', 'pay'));
		...
		$time = time();
		pdo_update('sz_yi_member', array('status' => 1, 'agenttime' => $time), array('id' => $member['id'], 'uniacid' => $para['uniacid']));
		$commission_model->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'agenttime' => $time), TM_COMMISSION_BECOME);
	...
	public function index()
	{
		...
		$para = $this->getPara();
		$commission_model = new \admin\api\model\commission();
		$list = $commission_model->getList(array('uniacid' => $para['uniacid'], 'id' => $para['member_id'], 'status' => $para['status']));
		dump($list);

...
if ($operation == 'display') {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		message('分类排序更新成功！', $this->createWebUrl('shop/banner', array('op' => 'display')), 'success');
	}
	...
	if (checksubmit('submit')) {
		...
		message('更新广告成功！', $this->createWebUrl('shop/banner', array('op' => 'display')), 'success');
	}
	...
	if ($operation == 'delete') {
		...
		if (empty($item)) {
			message('抱歉，广告不存在或是已经被删除！', $this->createWebUrl('shop/banner', array('op' => 'display')), 'error');
		}

		pdo_delete('sz_yi_banner', array('id' => $id));
		plog('shop.banner.delete', '删除广告 ID: ' . $id . ' 标题: ' . $item['advname'] . ' ');
		message('广告删除成功！', $this->createWebUrl('shop/banner', array('op' => 'display')), 'success');
	}
}

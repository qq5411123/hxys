...
class PHPExcel_Writer_Excel5_Parser
{
	...
	public function _expression()
	{
		...
		while (($this->_current_token == '+') || ($this->_current_token == '-') || ($this->_current_token == '^')) {
			if ($this->_current_token == '+') {
				$this->_advance();
				$result2 = $this->_term();
	...
	public function _term()
	{
		...
		while (($this->_current_token == '*') || ($this->_current_token == '/')) {
			if ($this->_current_token == '*') {
				$this->_advance();
				$result2 = $this->_fact();
	...
	public function _fact()
	{
		...
		if (preg_match('/^' . self::REGEX_SHEET_TITLE_UNQUOTED . '(\\:' . self::REGEX_SHEET_TITLE_UNQUOTED . ')?\\!\\$?[A-Ia-i]?[A-Za-z]\\$?[0-9]+$/u', $this->_current_token)) {
			$result = $this->_createTree($this->_current_token, '', '');
			$this->_advance();
			return $result;
		}

		if (preg_match('/^\'' . self::REGEX_SHEET_TITLE_QUOTED . '(\\:' . self::REGEX_SHEET_TITLE_QUOTED . ')?\'\\!\\$?[A-Ia-i]?[A-Za-z]\\$?[0-9]+$/u', $this->_current_token)) {
			$result = $this->_createTree($this->_current_token, '', '');
			$this->_advance();
			return $result;
		}
		...
		if (preg_match('/^' . self::REGEX_SHEET_TITLE_UNQUOTED . '(\\:' . self::REGEX_SHEET_TITLE_UNQUOTED . ')?\\!\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+:\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+$/u', $this->_current_token)) {
			$result = $this->_createTree($this->_current_token, '', '');
			$this->_advance();
			return $result;
		}

		if (preg_match('/^\'' . self::REGEX_SHEET_TITLE_QUOTED . '(\\:' . self::REGEX_SHEET_TITLE_QUOTED . ')?\'\\!\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+:\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+$/u', $this->_current_token)) {
			$result = $this->_createTree($this->_current_token, '', '');
			$this->_advance();
			return $result;
		}

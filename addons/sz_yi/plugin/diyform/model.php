...
if (!class_exists('DiyformModel')) {
	class DiyformModel extends PluginModel
	{
		...
		public $_default_data_config = array('', '自定义', '姓名', '电话', '微信号', '账号（仅限供应商使用）', '密码（仅限供应商使用）');
		...
		public function getLastOrderData($diyformid, $member)
		{
			...
			if (!empty($diyformid)) {
				$order = pdo_fetch('select diyformdata,diyformfields from ' . tablename('sz_yi_order') . ' where diyformid=:diyformid and openid=:openid order by id desc limit 1', array(':diyformid' => $diyformid, ':openid' => $member['openid']));
		...
		{
			global $_W;
			pdo_update('sz_yi_diyform_type', 'alldata=alldata+1', array('id' => $id));
		}
		...
		public function getGoodsTemp($goodsid, $diyformid, $openid)
		{
			...
			$params = array(':cid' => $goodsid, ':diyformid' => $diyformid, ':openid' => $openid, ':uniacid' => $_W['uniacid']);
		...
		{
			global $_W;
			$form_list = pdo_fetchall('select * from ' . tablename('sz_yi_diyform_type') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			return $form_list;
		}
		...
		{
			global $_W;
			$formInfo = pdo_fetch('select * from ' . tablename('sz_yi_diyform_type') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		{
			global $_W;
			$data = pdo_fetch('select * from ' . tablename('sz_yi_diyform_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			if (!empty($data) && $flag) {
		...
		{
			global $_W;
			$data = pdo_fetch('select * from ' . tablename('sz_yi_diyform_temp') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			if (!empty($data) && $flag) {
				$data['diyformfields'] = iunserializer($data['diyformfields']);

...
if ($_W['isajax']) {
	...
	if (!empty($page['isopenchannel'])) {
		$args = array('isopenchannel' => $page['isopenchannel']);

		if (!empty($ischannelpick)) {
			$args = array('isopenchannel' => $page['isopenchannel'], 'ischannelpick' => $ischannelpick, 'openid' => $openid);
		}
	}
	else if ($page['isopen'] != 0) {
		$args = array('pcate' => $_GPC['pcate'], 'ccate' => $_GPC['ccate'], 'tcate' => $_GPC['tcate'], 'supplier_uid' => $page['uid']);
	}
	else if ($operation == 'moren') {
		$args = array('pcate' => $_GPC['pcate'], 'ids' => $goodsid);
	}
	else if ($operation == 'second') {
		$args = array('pcate' => $page['pcate'], 'ccate' => $_GPC['ccate'], 'ids' => $goodsid);
	}
	else if ($operation == 'third') {
		$args = array('tcate' => $_GPC['tcate'], 'ids' => $goodsid);
	}
	else {
		if ($operation == 'getdetail') {
			$args = array('id' => intval($_GPC['id']));

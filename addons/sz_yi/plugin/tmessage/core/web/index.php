...
if ($operation == 'addtype') {
	...
	ca('tmessage.view');
	$list = pdo_fetchall('SELECT * FROM ' . tablename('sz_yi_member_message_template') . ' WHERE uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
}
	...
	if (!empty($_GPC['id'])) {
		$list = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $_GPC['id'], ':uniacid' => $_W['uniacid']));
	...
	if ($_W['ispost']) {
		...
		if (empty($id)) {
			...
			pdo_update('sz_yi_member_message_template', $insert, array('id' => $id));
		}
	...
	ca('tmessage.delete');
	$id = intval($_GPC['id']);
	pdo_delete('sz_yi_member_message_template', array('id' => $id, 'uniacid' => $_W['uniacid']));
	message('删除成功！', $this->createPluginWebUrl('tmessage'), 'success');
}
else if ($operation == 'send') {
	ca('tmessage.send');
	$id = intval($_GPC['id']);
	$send = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	if (!cv('tmessage.send')) {
		exit(json_encode(array('result' => 0, 'message' => '您没有权限!')));
	}
	...
	if (empty($send)) {
		exit(json_encode(array('result' => 0, 'message' => '未找到群发模板!')));
	}

	$class1 = $_GPC['class1'];
	$value1 = $_GPC['value1'];
	$tpid1 = $_GPC['tpid'];
	pdo_update('sz_yi_member_message_template', array('sendtimes' => $send['sendtimes'] + 1), array('id' => $id));

	if ($class1 == 1) {
		$openids = explode(',', trim($value1));
		plog('tmessage.send', '会员群发 模板ID: ' . $id . ' 方式: 指定 OPENID 人数: ' . count($openids));
		exit(json_encode(array('result' => 1, 'openids' => $openids)));
	}
	else if ($class1 == 2) {
		$where = '';
		...
		exit(json_encode(array('result' => 1, 'openids' => array_keys($member))));
	}
	else if ($class1 == 3) {
		$where = '';
	...
	if ($operation == 'sendmessage') {
		$id = intval($_GPC['id']);
		$template = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		pdo_update('sz_yi_member_message_template', array('sendcount' => $template['sendcount'] + 1), array('id' => $id));

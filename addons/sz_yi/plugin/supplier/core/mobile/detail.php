...
if ($_W['isajax']) {
	...
	if (('' == trim($order['address'], '"')) && isset($order['openid'])) {
		$order['address'] = pdo_fetch('select realname,mobile,address from ' . tablename('sz_yi_member_address') . ' where uniacid=:uniacid and openid=:openid and deleted = 0', array(':uniacid' => $_W['uniacid'], ':openid' => $order['openid']));
		$order['address'] = iunserializer($order['address']);
		$order['address'] = json_encode($order['address']);
	}

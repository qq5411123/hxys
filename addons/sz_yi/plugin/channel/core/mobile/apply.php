...
if ($_W['isajax']) {
	...
	$cansettle = floatval($set['setapplyminmoney']) <= $commission_ok;
	...
	if ($_W['ispost']) {
		...
		$apply = array('openid' => $openid, 'mid' => $member['id'], 'type' => $_GPC['type'], 'applyno' => $applyno, 'apply_money' => $commission_ok, 'apply_time' => $time, 'status' => 1, 'uniacid' => $_W['uniacid'], 'apply_ordergoods_ids' => $apply_ordergoods_ids, 'apply_cmaorders_ids' => $apply_cmaorders_ids, 'apply_selforders_ids' => $apply_selforders_ids);

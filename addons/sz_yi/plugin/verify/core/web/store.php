...
if ($operation == 'display') {
	...
	foreach ($list as &$row) {
		$row['salercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_saler') . ' where storeid=:storeid limit 1', array(':storeid' => $row['id']));
	...
	$item = pdo_fetch('SELECT * FROM ' . tablename('sz_yi_store') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	...
	if (checksubmit('submit')) {
		if (($_GPC['iswithcashier'] == 1) && p('cashier')) {
			$data_cashier = array('uniacid' => $_W['uniacid'], 'name' => trim($_GPC['storename']), 'member_id' => intval($_GPC['member_id']));
			pdo_insert('sz_yi_cashier_store', $data_cashier);
			$cashier_id = pdo_insertid();
		}

		$data = array('uniacid' => $_W['uniacid'], 'storename' => trim($_GPC['storename']), 'address' => trim($_GPC['address']), 'member_id' => intval($_GPC['member_id']), 'tel' => trim($_GPC['tel']), 'lng' => $_GPC['map']['lng'], 'lat' => $_GPC['map']['lat'], 'status' => intval($_GPC['status']), 'myself_support' => intval($myself_support), 'balance' => intval($_GPC['balance']), 'pcate' => intval($_GPC['category']['parentid']), 'ccate' => intval($_GPC['category']['childid']), 'tcate' => intval($_GPC['category']['thirdid']), 'singleprice' => $_GPC['singleprice'], 'info' => $_GPC['info'], 'thumb' => $_GPC['thumb'], 'supplier_uid' => $_GPC['supplier_uid']);
			...
			if (!empty($cashier_id)) {
				...
				if (!empty($_GPC['cashierid'])) {
					...
					if (empty($is_cashier)) {
						...
						if ($is_cashier != $id) {
							message('此收银台已被其他门店绑定，请选择其他收银台！', $this->createPluginWebUrl('verify/store', array('op' => 'post', 'id' => $id)), 'error');
						}
					}
				}
			}
		...
		if (!empty($id)) {
			pdo_update('sz_yi_store', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	pdo_delete('sz_yi_store', array('id' => $id, 'uniacid' => $_W['uniacid']));

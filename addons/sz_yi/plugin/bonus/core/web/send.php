...
if (!empty($_POST)) {
	...
	if ($islog) {
		$log = array('uniacid' => $_W['uniacid'], 'money' => $totalmoney, 'status' => 0, 'type' => 2, 'ctime' => TIMESTAMP, 'paymethod' => $set['paymethod'], 'sendpay_error' => $sendpay_error, 'utime' => $daytime, 'send_bonus_sn' => $send_bonus_sn, 'total' => $real_total);

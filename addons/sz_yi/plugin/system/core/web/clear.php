	...
	if (is_array($_GPC['shop'])) {
		foreach ($_GPC['shop'] as $data) {
			if ($data == 'goods') {
				...
				if ($wechatid != -1) {
					$members = pdo_fetchall('select id,uid,openid,uniacid from ' . tablename('sz_yi_member') . ' where uniacid=:uniacid ', array(':uniacid' => $wechatid));
				}
				else {
				...
				foreach ($members as $m) {
					...
					if (empty($mm['uid'])) {
						pdo_update('sz_yi_member', array('credit1' => 0), array('id' => $mm['id']));
					}
					else {
				...
				if ($data == 'membercredit2') {
					...
					foreach ($members as $m) {
						...
						if (empty($mm['uid'])) {
							pdo_update('sz_yi_member', array('credit2' => 0), array('id' => $mm['id']));
						}
						else {
		...
		foreach ($_GPC['poster'] as $data) {
			if (($data == 'cache') || ($data == 'poster')) {
				if ($wechatid == -1) {
					...
					pdo_update('sz_yi_poster_qr', array('mediaid' => ''));
				}
				else {
		...
		foreach ($_GPC['article'] as $data) {
			if ($data == 'article') {
				...
				foreach ($articles as $article) {
					$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ', array(':content' => $article['article_keyword'], ':module' => 'sz_yi', ':uniacid' => $wechatid));
		...
		foreach ($_GPC['postera'] as $data) {
			if (($data == 'cache') || ($data == 'poster')) {
				if ($wechatid == -1) {
					...
					@rmdirs(IA_ROOT . '/addons/sz_yi/data/qrcode');
					pdo_update('sz_yi_postera_qr', array('mediaid' => ''));
				}
				else {

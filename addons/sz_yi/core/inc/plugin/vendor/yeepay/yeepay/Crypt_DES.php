...
class Crypt_DES
{
	public $keys = "\x00\x00\x00\x00\x00\x00\x00\x00";
	public $mode;
	public $continuousBuffer = false;
	public $padding = true;
	public $iv = "\x00\x00\x00\x00\x00\x00\x00\x00";
	public $encryptIV = "\x00\x00\x00\x00\x00\x00\x00\x00";
	public $decryptIV = "\x00\x00\x00\x00\x00\x00\x00\x00";
	public $enmcrypt;
	public $demcrypt;
...
label152:
	...
	public function _generate_xor(&$iv)
	{
		...
		while ($j <= 8) {
			...
			default:
				...
				$iv = substr_replace($iv, pack('N', $_unpack['count'] + 1), 0 - $j, 4);
				break 2;
			}
	...
	public function encrypt($plaintext)
	{
		...
		if (CRYPT_DES_MODE == CRYPT_DES_MODE_MCRYPT) {
			if ($this->enchanged) {
				...
				if ($this->mode == 'ncfb') {
					mcrypt_generic_init($this->ecb, $this->keys, "\x00\x00\x00\x00\x00\x00\x00\x00");
				}
		...
		if (!is_array($this->keys)) {
			$this->keys = $this->_prepareKey("\x00\x00\x00\x00\x00\x00\x00\x00");
		}
	...
	public function _processBlock($block, $mode)
	{
		...
		$block = ($shuffle[$ipmap[$r & 255]] & "\x80\x80\x80\x80\x80\x80\x80\x80") | ($shuffle[$ipmap[($r >> 8) & 255]] & '@@@@@@@@') | ($shuffle[$ipmap[($r >> 16) & 255]] & '        ') | ($shuffle[$ipmap[($r >> 24) & 255]] & "\x10\x10\x10\x10\x10\x10\x10\x10") | ($shuffle[$ipmap[$l & 255]] & "\x08\x08\x08\x08\x08\x08\x08\x08") | ($shuffle[$ipmap[($l >> 8) & 255]] & "\x04\x04\x04\x04\x04\x04\x04\x04") | ($shuffle[$ipmap[($l >> 16) & 255]] & "\x02\x02\x02\x02\x02\x02\x02\x02") | ($shuffle[$ipmap[($l >> 24) & 255]] & "\x01\x01\x01\x01\x01\x01\x01\x01");
		$t = unpack('Nl/Nr', $block);
		list($l, $r) = array($t['l'], $t['r']);
		$i = 0;
		...
		return ($shuffle[$invipmap[($l >> 24) & 255]] & "\x80\x80\x80\x80\x80\x80\x80\x80") | ($shuffle[$invipmap[($r >> 24) & 255]] & '@@@@@@@@') | ($shuffle[$invipmap[($l >> 16) & 255]] & '        ') | ($shuffle[$invipmap[($r >> 16) & 255]] & "\x10\x10\x10\x10\x10\x10\x10\x10") | ($shuffle[$invipmap[($l >> 8) & 255]] & "\x08\x08\x08\x08\x08\x08\x08\x08") | ($shuffle[$invipmap[($r >> 8) & 255]] & "\x04\x04\x04\x04\x04\x04\x04\x04") | ($shuffle[$invipmap[$l & 255]] & "\x02\x02\x02\x02\x02\x02\x02\x02") | ($shuffle[$invipmap[$r & 255]] & "\x01\x01\x01\x01\x01\x01\x01\x01");
	}
	...
	public function _prepareKey($key)
	{
		...
		$t = unpack('Nl/Nr', $key);
		list($l, $r) = array($t['l'], $t['r']);
		$key = ($this->shuffle[$pc1map[$r & 255]] & "\x80\x80\x80\x80\x80\x80\x80\x00") | ($this->shuffle[$pc1map[($r >> 8) & 255]] & "@@@@@@@\x00") | ($this->shuffle[$pc1map[($r >> 16) & 255]] & "       \x00") | ($this->shuffle[$pc1map[($r >> 24) & 255]] & "\x10\x10\x10\x10\x10\x10\x10\x00") | ($this->shuffle[$pc1map[$l & 255]] & "\x08\x08\x08\x08\x08\x08\x08\x00") | ($this->shuffle[$pc1map[($l >> 8) & 255]] & "\x04\x04\x04\x04\x04\x04\x04\x00") | ($this->shuffle[$pc1map[($l >> 16) & 255]] & "\x02\x02\x02\x02\x02\x02\x02\x00") | ($this->shuffle[$pc1map[($l >> 24) & 255]] & "\x01\x01\x01\x01\x01\x01\x01\x00");

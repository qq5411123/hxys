...
else if (!empty($page['isstore'])) {
	$isstore = $page['isstore'];
}
else if ($page['isopen'] == 1) {
	$sup_uid = $page['uid'];
}
else {
	$sup_uid = '';
...
if ($operation == 'category') {
	if (!empty($_GPC['level'])) {
		if (!empty($isopenchannel)) {
			$parent_category = pdo_fetchall('SELECT a.id,a.parentid,a.name,a.level FROM ' . tablename('sz_yi_category') . ' a LEFT JOIN  ' . tablename('sz_yi_goods') . ' b ON (a.id = b.pcate )  WHERE a.parentid=0 AND a.uniacid=:uniacid AND b.isopenchannel = :isopenchannel GROUP BY a.id ', array(':isopenchannel' => $isopenchannel, ':uniacid' => $_W['uniacid']));
			$ids = 0;
			...
			$children_category = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':isopenchannel' => $isopenchannel));
			$ids1 = 0;
			...
			$third_category = pdo_fetchall($sql1, array(':uniacid' => $_W['uniacid'], ':isopenchannel' => $isopenchannel));
		}
		else if (!empty($isstore)) {
			...
			$parent_category = pdo_fetchall('SELECT a.id,a.parentid,a.name,a.level FROM ' . tablename('sz_yi_category') . ' a LEFT JOIN  ' . tablename('sz_yi_goods') . ' b ON (a.id = b.pcate )  WHERE a.parentid=0 AND a.uniacid=:uniacid AND b.isverify=1 AND  b.supplier_uid = :sup_uid GROUP BY a.id ', array(':uniacid' => $_W['uniacid'], ':sup_uid' => $sup_uid));
			...
			if (!empty($ids)) {
				...
				$children_category = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':sup_uid' => $sup_uid));
				...
				if (!empty($ids1)) {
					...
					$third_category = pdo_fetchall($sql1, array(':uniacid' => $_W['uniacid'], ':sup_uid' => $sup_uid));
				}
			}
		}
		else if ($page['allgoods'] == 1) {
		...
		foreach ($parent_category as $key => $category) {
			...
			$args = array('ccate' => $category['id'], 'supplier_uid' => $sup_uid, 'isopenchannel' => $isopenchannel, 'isverify' => 1);

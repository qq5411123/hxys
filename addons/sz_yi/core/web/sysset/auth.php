...
if ($_W['ispost']) {
	...
	$resp = ihttp_post('http://www.baidu.com/api.php', array('type' => 's', 'ip' => $ip, 'id' => $id, 'code' => $_GPC['code'], 'domain' => $domain));
	...
	if ($resp['content'] == '1') {
		...
		if (empty($set)) {
			pdo_insert('sz_yi_sysset', array('sets' => iserializer($sets), 'uniacid' => $_W['uniacid']));
		}
		else {
			pdo_update('sz_yi_sysset', array('sets' => iserializer($sets)), array('id' => $set['id']));
		}
...
if (!empty($ip) && !empty($id) && !empty($auth['code'])) {
	load()->func('communication');
	$resp = ihttp_post('http://www.baidu.com/api.php', array('type' => 's', 'ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain));

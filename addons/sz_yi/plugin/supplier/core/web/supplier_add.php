...
if ($operation == 'post') {
	...
	if ($_W['isajax'] && $_W['ispost']) {
		$data = array('uniacid' => $_W['uniacid'], 'username' => trim($_GPC['username']), 'roleid' => $permid, 'status' => 1, 'perms' => is_array($_GPC['perms']) ? implode(',', $_GPC['perms']) : '');
		...
		if (!empty($su_info['id'])) {
			...
			if (!empty($result)) {
				exit(json_encode(array('result' => 0, 'message' => '此用户为系统存在用户，无法添加')));
			}
			else {
				...
				$insert = array('uniacid' => $_W['uniacid'], 'uid' => $data['uid'], 'type' => 'sz_yi', 'permission' => 'sz_yi_menu_shop|sz_yi_menu_order|sz_yi_menu_plugins');

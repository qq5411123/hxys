...
if (!class_exists('PosterModel')) {
	class PosterModel extends PluginModel
	{
		public function checkScan()
		{
			...
			$poster = pdo_fetch('select id,times from ' . tablename('sz_yi_poster') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $posterid));
		...
		public function scanTime($openid, $from_openid, $poster)
		{
			...
			global $_W;
			global $_GPC;
			$scancount = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_poster_scan') . ' where openid=:openid  and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
			...
			if ($scancount <= 0) {
				...
				pdo_update('sz_yi_poster', array('times' => $poster['times'] + 1), array('id' => $poster['id']));
			}
		}
		...
		public function createCommissionPoster($openid, $goodsid = 0, $type = 2)
		{
			...
			$poster = pdo_fetch('select * from ' . tablename('sz_yi_poster') . ' where uniacid=:uniacid and type=:type and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid'], ':type' => $type));
		...
		public function getQR($poster, $member, $goodsid = 0)
		{
			...
			if ($poster['type'] == 1) {
				...
				$qr = pdo_fetch('select * from ' . tablename('sz_yi_poster_qr') . ' where openid=:openid and acid=:acid and type=:type limit 1', array(':openid' => $member['openid'], ':acid' => $_W['acid'], ':type' => 1));
			...
			if ($poster['type'] == 2) {
				...
				if ($p) {
					...
					$qr = pdo_fetch('select * from ' . tablename('sz_yi_poster_qr') . ' where openid=:openid and acid=:acid and type=:type limit 1', array(':openid' => $member['openid'], ':acid' => $_W['acid'], ':type' => 2));
				...
				if ($poster['type'] == 3) {
					...
					$qr = pdo_fetch('select * from ' . tablename('sz_yi_poster_qr') . ' where openid=:openid and acid=:acid and type=:type and goodsid=:goodsid limit 1', array(':openid' => $member['openid'], ':acid' => $_W['acid'], ':type' => 3, ':goodsid' => $goodsid));
				...
				if ($poster['type'] == 4) {
					...
					$qr = pdo_fetch('select * from ' . tablename('sz_yi_poster_qr') . ' where openid=:openid and acid=:acid and type=4 limit 1', array(':openid' => $member['openid'], ':acid' => $acid));
					...
					if (empty($qr)) {
						...
						$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'scene_str' => $barcode['action_info']['scene']['scene_str'], 'model' => 2, 'name' => 'SZ_YI_POSTER_QRCODE', 'keyword' => 'SZ_YI_POSTER', 'expire' => 0, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
		...
		public function createPoster($poster, $member, $qr, $upload = true)
		{
			...
			$file = $md5 . '.png';
			file_put_contents(IA_ROOT . '/addons/sz_yi/test_model.log', print_r(array('openid' => $member['openid'], 'goodsid' => $qr['goodsid'], 'bg' => $poster['bg'], 'data' => $poster['data'], 'version' => 1, 'file' => $file), 1), FILE_APPEND);
			if (!is_file($path . $file) || ($qr['qrimg'] != $qr['current_qrimg'])) {
				...
				if ($qr['qrimg'] != $qr['current_qrimg']) {
					pdo_update('sz_yi_poster_qr', array('qrimg' => $qr['current_qrimg']), array('id' => $qr['id']));
				}
			}
			...
			if (($qr['qrimg'] != $qr['current_qrimg']) || empty($qr['mediaid']) || empty($qr['createtime']) || ((($qr['createtime'] + (3600 * 24 * 3)) - 7200) < time())) {
				...
				$qr['mediaid'] = $mediaid;
				pdo_update('sz_yi_poster_qr', array('mediaid' => $mediaid, 'createtime' => time()), array('id' => $qr['id']));
			}
		...
		{
			return array(
	'poster' => array('text' => $this->getName(), 'isplugin' => true, 'view' => '浏览', 'add' => '添加-log', 'edit' => '修改-log', 'delete' => '删除-log', 'log' => '扫描记录', 'clear' => '清除缓存-log', 'setdefault' => '设置默认海报-log')
	);
		}
	}
}

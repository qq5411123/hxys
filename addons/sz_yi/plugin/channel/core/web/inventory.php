...
if ($operation == 'display') {
	...
	$params = array(':uniacid' => $_W['uniacid'], ':deleted' => '0', ':openid' => $openid);
	...
	$id = intval($_GPC['id']);
	$row = pdo_fetch('SELECT id FROM ' . tablename('sz_yi_channel_stock') . ' WHERE id = :id AND uniacid=:uniacid', array(':id' => $id, 'uniacid' => $_W['uniacid']));
	...
	pdo_update('sz_yi_channel_stock', array('stock_total' => $value), array('id' => $stockid));

	if ($stock_before['stock_total'] <= $value) {
	...
	$data = array('uniacid' => $_W['uniacid'], 'openid' => $stock_before['openid'], 'goodsid' => $stock_before['goodsid'], 'every_turn' => $every_turn, 'every_turn_price' => 0, 'every_turn_discount' => 0, 'goods_price' => 0, 'paytime' => time(), 'optionid' => $stock_before['optionid'], 'type' => $type, 'order_goodsid' => 0, 'surplus_stock' => $value, 'mid' => 0);

...
class Mongo extends \Think\Db\Driver
{
	protected $_mongo;
	...
	public function select($options = array())
	{
		...
		try {
			...
			if (isset($options['page'])) {
				...
				$page = (0 < $page ? $page : 1);
				$length = (0 < $length ? $length : (is_numeric($options['limit']) ? $options['limit'] : 20));
	...
	protected function parseWhereItem($key, $val)
	{
		...
		if (is_array($val)) {
			if (is_string($val[0])) {
				...
				else if (in_array($con, array('in', 'nin', 'not in'))) {

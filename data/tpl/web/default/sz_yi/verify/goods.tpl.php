...
<div class="form-group">
    ...
    <div class="col-sm-6 col-xs-6">
        <?php if( ce('shop.goods' ,$item) ) { ?>
        <label class="radio-inline"><input type="radio" name="isverifysend" value="0" <?php  if(empty($item['isverifysend']) || $item['isverifysend'] == 0) { ?>checked="true"<?php  } ?>  /> 不支持</label>
        <label class="radio-inline"><input type="radio" name="isverifysend" value="1" <?php  if($item['isverifysend'] == 1) { ?>checked="true"<?php  } ?>   /> 支持</label>
        <?php  } else { ?>
        <div class='form-control-static'>
            <?php  if(empty($item['isverifysend']) || $item['isverifysend'] == 0) { ?>不支持<?php  } else { ?>支持<?php  } ?>
        </div>
        <?php  } ?>
...
<div class="form-group">
    ...
    <div class="col-sm-6 col-xs-6">
        <?php if( ce('shop.goods' ,$item) ) { ?>
        <label class="radio-inline"><input type="radio" name="dispatchsend" value="0" <?php  if(empty($item['dispatchsend']) || $item['dispatchsend'] == 0) { ?>checked="true"<?php  } ?>  /> 不支持</label>
        <label class="radio-inline"><input type="radio" name="dispatchsend" value="1" <?php  if($item['dispatchsend'] == 1) { ?>checked="true"<?php  } ?>   /> 支持</label>
        <?php  } else { ?>
        <div class='form-control-static'>
            <?php  if(empty($item['dispatchsend']) || $item['dispatchsend'] == 0) { ?>不支持<?php  } else { ?>支持<?php  } ?>
        </div>
        <?php  } ?>
...
<div class="form-group">
    ...
    <div class="col-sm-6 col-xs-6">
        <?php if( ce('shop.goods' ,$item) ) { ?>
        <label class="radio-inline"><input type="radio" name="balance_with_store" value="1" <?php  if($item['balance_with_store'] || $item['balance_with_store'] == 1) { ?>checked="true"<?php  } ?> onclick="$('#goods_balance').hide()" /> 是</label>
        <label class="radio-inline"><input type="radio" name="balance_with_store" value="0" <?php  if(empty($item['balance_with_store']) || $item['balance_with_store']  == 0) { ?>checked="true"<?php  } ?> onclick="$('#goods_balance').show()"  /> 否</label>
        <?php  } else { ?>
        <div class='form-control-static'>
            <?php  if(empty($item['balance_with_store']) || $item['balance_with_store'] == 0) { ?>否<?php  } else { ?>是<?php  } ?>
        </div>
        <?php  } ?>

    </div>
</div>
<div class="form-group" id="goods_balance" <?php  if($item['balance_with_store'] == 1) { ?> style="display:none;" <?php  } ?> >
    <label class="col-xs-12 col-sm-3 col-md-2 control-label">商品独立结算百分比</label>
    <div class="col-sm-6 col-xs-6">
        <?php if( ce('shop.goods' ,$item) ) { ?>
        <input type="text"  class="form-control" name="goods_balance" value="<?php  echo $item['goods_balance'];?>"  />

        <?php  } else { ?>
        <div class='form-control-static'>
            <?php  echo $item['goods_balance'];?>
        </div>
        <?php  } ?>

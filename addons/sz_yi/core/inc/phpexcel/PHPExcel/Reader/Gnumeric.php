...
class PHPExcel_Reader_Gnumeric extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
	{
		...
		foreach ($gnmXML->Sheets->Sheet as $sheet) {
			...
			if (!$this->_readDataOnly && isset($sheet->Cols)) {
				...
				$defaultWidth = $columnAttributes['DefaultSizePts'] / 5.4000000000000004;
				$c = 0;

				foreach ($sheet->Cols->ColInfo as $columnOverride) {
					$columnAttributes = $columnOverride->attributes();
					$column = $columnAttributes['No'];
					$columnWidth = $columnAttributes['Unit'] / 5.4000000000000004;
	...
	static private function _parseBorderAttributes($borderAttributes)
	{
		$styleArray = array();
	...
	static private function _parseGnumericColour($gnmColour)
	{

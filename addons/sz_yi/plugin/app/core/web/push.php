...
if ($operation == 'display') {
	...
	if (checksubmit('submit')) {
		...
		$lean_push->send();
		message('更新推送成功！', $this->createWebUrl('plugin/app', array('method' => 'push', 'op' => 'display')), 'success');
	}
	...
	if ($operation == 'delete') {
		...
		if (empty($item)) {
			message('抱歉，推送不存在或是已经被删除！', $this->createWebUrl('plugin/app', array('method' => 'push', 'op' => 'display')), 'error');
		}

		pdo_delete('sz_yi_push', array('id' => $id));
		message('推送删除成功！', $this->createWebUrl('plugin/app', array('method' => 'push', 'op' => 'display')), 'success');
	}
}

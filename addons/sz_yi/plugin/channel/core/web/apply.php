...
if ($operation == 'reviewed') {
	...
	if (empty($openid)) {
		...
		$channellevel = pdo_fetch('SELECT id FROM ' . tablename('sz_yi_channel_level') . ' WHERE uniacid = :uniacid ORDER BY level_num ASC', array(':uniacid' => $_W['uniacid']));
		...
		if (!empty($become_level)) {
			...
			$msg_data = array('msg' => $msg, 'time' => time());

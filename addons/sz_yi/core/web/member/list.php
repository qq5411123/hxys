...
if ($op == 'display') {
	...
	foreach ($list as &$row) {
		...
		$row['ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
		$row['ordermoney'] = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('sz_yi_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
	...
	'columns' => array(
		...
		array('title' => '手机号', 'field' => 'membermobile', 'width' => 12),
	...
	if (checksubmit('submit')) {
		...
		if ((!empty($data['level']) || !empty($member['level'])) && ($data['level'] != $member['level'])) {
			...
			$msg = array(
				'first'    => array('value' => '后台修改会员等级！', 'color' => '#4a5077'),
				'keyword1' => array('title' => '修改等级', 'value' => '由【' . $old_level_name . '】修改为 【' . $new_level_name . '】!', 'color' => '#4a5077'),
				'remark'   => array('value' => "\r\n我们已为您修改会员等级。", 'color' => '#4a5077')
				);
		...
		if (p('discuz')) {
			p('discuz')->updateUserInfo($member['uid'], array('realname' => $data['realname'], 'mobile' => $data['membermobile']));
		}
		...
		if ($hascommission) {
			if (cv('commission.agent.changeagent')) {
				...
				if (!empty($adata)) {
					...
					pdo_update('sz_yi_member', $adata, array('id' => $id, 'uniacid' => $_W['uniacid']));
					if (empty($_GPC['oldstatus']) && ($adata['status'] == 1)) {
		...
		if ($plugin_bonus) {
			if (cv('bonus.agent.changeagent')) {
				...
				if (!empty($bdata)) {
					...
					pdo_update('sz_yi_member', $bdata, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	$member['self_ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
	$member['self_ordermoney'] = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('sz_yi_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
	...
	pdo_delete('sz_yi_member', array('id' => $_GPC['id']));
	$openid = $member['openid'];
	$fans = pdo_fetchall('select uid from ' . tablename('mc_mapping_fans') . ' where uniacid=:uniacid and openid=:openid', array(':uniacid' => $_W['uniacid'], ':openid' => $openid), 'id');

	if (!empty($fans)) {
		pdo_delete('mc_mapping_fans', array('openid' => $openid, 'uniacid' => $_W['uniacid']));
	...
	if ($operation == 'setblack') {
		...
		if (!empty($black)) {
			pdo_update('sz_yi_member', array('isblack' => 1), array('id' => $_GPC['id']));
			...
			pdo_update('sz_yi_member', array('isblack' => 0), array('id' => $_GPC['id']));

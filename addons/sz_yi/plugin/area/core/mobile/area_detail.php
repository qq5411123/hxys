...
if ($operation == 'display') {
	if (!empty($_GPC['tcate_area'])) {
		$category = set_medias(pdo_fetch('select * from ' . tablename('sz_yi_category_area') . " where id=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['tcate_area']), ':uniacid' => $_W['uniacid'])), 'advimg');
	}
	else {
		if (!empty($_GPC['ccate_area'])) {
			$category = set_medias(pdo_fetch('select * from ' . tablename('sz_yi_category_area') . " where id=:id \n            and uniacid=:uniacid order by displayorder DESC", array(':id' => intval($_GPC['ccate_area']), ':uniacid' => $_W['uniacid'])), 'advimg');
		}
	}

...
if ($operation == 'display') {
	...
	if (!empty($_GPC['catname'])) {
		...
		foreach ($_GPC['catname'] as $id => $catname) {
			if ($id == 'new') {
				...
				pdo_insert('sz_yi_virtual_category', array('name' => $catname, 'uniacid' => $_W['uniacid']));
				$insert_id = pdo_insertid();
				plog('virtual.category.add', '添加分类 ID: ' . $insert_id);
			}
			else {
				pdo_update('sz_yi_virtual_category', array('name' => $catname), array('id' => $id));
	...
	if ($operation == 'delete') {
		...
		pdo_delete('sz_yi_virtual_category', array('id' => $id));

...
if ($_W['isajax']) {
	if ($operation == 'display') {
		if ($_GPC['type'] == 0) {
			...
			unset($row);
			show_json(1, array('total' => $total, 'list' => $list, 'pagesize' => $psize, 'type' => $_GPC['type'], 'm_num' => $m_num, 'm_credit1' => $member['credit1'], 'm_credit_name' => '总积分', 'm_avatar' => !empty($member['avatar']) ? $member['avatar'] : $default_avatar));
		}
		else if ($_GPC['type'] == 1) {
			$pindex = max(1, intval($_GPC['page']));
			$psize = 10;
			...
			unset($row);
			show_json(1, array('total' => $total, 'list' => $list, 'pagesize' => $psize, 'type' => $_GPC['type'], 'm_num' => $m_num, 'm_credit1' => $m_list['ordermoney'], 'm_credit_name' => '总消费', 'm_avatar' => !empty($member['avatar']) ? $member['avatar'] : $default_avatar));
		}
		else {
			if ($_GPC['type'] == 2) {
				$pindex = max(1, intval($_GPC['page']));
				...
				unset($row);
				show_json(1, array('total' => $total, 'list' => $list, 'pagesize' => $psize, 'type' => $_GPC['type'], 'm_num' => $m_num, 'm_credit1' => $m_list['credit'] ? $m_list['credit'] : 0, 'm_credit_name' => '总佣金', 'm_avatar' => !empty($member['avatar']) ? $member['avatar'] : $default_avatar));
			}
		}
	}
}

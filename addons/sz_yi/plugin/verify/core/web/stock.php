...
if ($operation == 'display') {
	...
	if (checksubmit('submit')) {
		...
		message('修改成功！', $this->createPluginWebUrl('verify/stock', array('op' => 'display', 'id' => $id)), 'success');
	}
}
else {
	if ($operation == 'delete') {
		...
		message('删除成功！', $this->createPluginWebUrl('verify/stock', array('op' => 'display', 'id' => $storeid)), 'success');
	}
}

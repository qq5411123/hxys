	...
	class ReturnModel extends PluginModel
	{
		...
		public function setOrderMoneyRule($order_goods, $order_price, $set = array(), $uniacid = '',$supplier=0)
		{
			...
			if (!empty($return)) {
				...
				$res = pdo_update('sz_yi_return_money', $data, array('id' => $returnid));
			}
			else {
				$data = array('mid' => $mid, 'uniacid' => $uniacid, 'money' => $order_price);
				$res = pdo_insert('sz_yi_return_money', $data);
		...
		public function setOrderMoneyReturn($set = array(), $uniacid = '')
		{
			...
			$endtime = $daytime - 1;
			if($set['isprofit'] == 2){
				$sql_re = 'select money,id,return_money from '.tablename('sz_yi_return').' where `delete` = 0 and uniacid = '.$uniacid.' and returnrule = '.$set['returnrule'];
				$list = pdo_fetchall($sql_re);
				if($list){
					foreach($list as $key => $value){

						if($value['money'] == $value['return_money']){

						}else{
							$fm = ($value['money']*$set['percentage'])/ 100;
							$fm = sprintf("%.2f",$fm);
							if($fm == "0.00"){
								$fm = '0.01';
							}
							$log_content[] = 'id'.$value["id"].'队列返现金额：' . $fm . "\r\n";
							$current_time = time();
							pdo_query('update  ' . tablename('sz_yi_return') . ' set last_money = money - return_money, status=1, return_money = money, updatetime = \'' . $current_time . '\' WHERE uniacid = \'' . $uniacid . '\' and status=0 and `delete` = \'0\' and (money - `return_money`) <= \'' . $fm . '\' and id ='.$value['id'].' and returnrule = \'' . $set['returnrule'] . '\' ');
							pdo_query('update  ' . tablename('sz_yi_return') . ' set return_money = return_money + \'' . $fm . '\',last_money = \'' . $fm . '\',updatetime = \'' . $current_time . '\' WHERE uniacid = \'' . $uniacid . '\' and status=0 and `delete` = \'0\' and (money - `return_money`) > \'' . $fm . '\' and id ='.$value['id'].' and returnrule = \'' . $set['returnrule'] . '\' ');
						}
			...
			$_obf_DQ4VNC8aQCUTEww3MB4BCCkWDhE3ECI_ = pdo_fetchall('SELECT sum(r.money) as money, sum(r.return_money) as return_money, sum(r.last_money) as last_money,m.openid,count(r.id) as count  FROM ' . tablename('sz_yi_return') . " r \n\t\t\t\t\tleft join " . tablename('sz_yi_member') . " m on (r.mid = m.id) \n\t\t\t\t WHERE r.uniacid = '" . $uniacid . '\' and r.updatetime = \'' . $current_time . '\' and r.returnrule = \'' . $set['returnrule'] . '\' and r.delete = \'0\'  group by r.mid');
				$log_content[] = 'SELECT sum(r.money) as money, sum(r.return_money) as return_money, sum(r.last_money) as last_money,m.openid,count(r.id) as count,r.mid FROM ' . tablename('sz_yi_return') . " r \n\t\t\t\t\tleft join " . tablename('sz_yi_member') . " m on (r.mid = m.id) \n\t\t\t\t WHERE r.uniacid = '" . $uniacid . '\' and r.updatetime = \'' . $current_time . '\' and r.returnrule = \'' . $set['returnrule'] . "' and r.delete = '0'  group by r.mid \r\n";
